...
static const char a_GapDeviceName[] = {  'D', 'i', 'r', 'e', 'c', 't', 'i', 'o', 'n', 'F', '_', 'T', 'a', 'g' }; /* Gap Device Name */
...
uint8_t a_AdvData[] =
{
  ...
  10, AD_TYPE_COMPLETE_LOCAL_NAME, 'D', 'F', '_', 'T', 'a', 'g', '_', 'X', 'X',  /* Complete name */
...
/* USER CODE BEGIN Role_Mngt*/
  APP_DBG_MSG("\n\n * * * * * Application name : BLE_DirectionFinding_Peripheral_Tag * * * * * *\n");
...
#error "Invalid CFG_BD_ADDRESS_TYPE"
  ...
  ret = aci_hal_set_antenna_switch_parameters(
    ANDENNA_IDS_BITS,  /* Bitmask for the pins for antenna switching to be configured (0x03, which is the OR of 0x01 ANTENNA_ID_0 and 0x02 ANTENNA_ID_1). */
    0,                 /* No shift for the ANTENNA_ID signal. */
    0,                 /* Default antenna ID for regular communication. */
    1                  /* Enable the RF Activity signal. */
  );
...
void APP_BLE_Init(void)
{
  ...
  */
  ...
  /* Create timer to handle the Led3 Switch OFF */
                                      ...
                                      uint16_t Supervision_Timeout)
{
  ...
  /* Set the CTE transmit parameters */
  ret = hci_le_set_connection_cte_transmit_parameters(
      Connection_Handle,          // Connection handle that identifies the connection
      CTE_TYPE_SUPPORT,           // Support for any CTE type
      TX_SWITCHING_PATTERN_LENGTH,// Length of the antenna switching pattern
  ...
  {
    APP_DBG_MSG("==>> hci_le_set_connection_cte_transmit_parameters : Success\n");
  }

  /* Enable the CTE response */
  ret = hci_le_connection_cte_response_enable(Connection_Handle, ENABLE);

  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("Failed to enable CTE response: reason=0x%02X\n", ret);
  }
  else
  {
    APP_DBG_MSG("==>> hci_le_connection_cte_response_enable : Success\n");
  }
