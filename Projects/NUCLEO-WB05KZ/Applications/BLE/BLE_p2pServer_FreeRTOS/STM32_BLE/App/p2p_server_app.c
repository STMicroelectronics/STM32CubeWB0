...
#define SEND_NOTIF_TASK_STACK_SIZE      (800)
#define SEND_NOTIF_TASK_PRIORITY        (tskIDLE_PRIORITY + 1)
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
TaskHandle_t SendNotifTaskHandle;
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP */
void P2P_SERVER_Switch_c_SendNotification_Task(void *pvParameters);
...
void P2P_SERVER_APP_Init(void)
{
  ...
  xTaskCreate(P2P_SERVER_Switch_c_SendNotification_Task, "SEND_NOTIF", SEND_NOTIF_TASK_STACK_SIZE>>2, NULL, SEND_NOTIF_TASK_PRIORITY, &SendNotifTaskHandle);
