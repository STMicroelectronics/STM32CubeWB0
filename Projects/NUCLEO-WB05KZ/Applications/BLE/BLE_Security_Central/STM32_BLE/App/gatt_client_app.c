...
typedef enum
{
  ...
  SECURITY_START_TIMER_EVT,
  SECURITY_STOP_TIMER_EVT,
  SECURITY_NOTIFICATION_INFO_RECEIVED_EVT,
...
typedef struct
{
  ...
  /* handles of the Security service */
  uint16_t SecurityServiceHdl;
  uint16_t SecurityServiceEndHdl;

  /* handles of the Tx characteristic - Write To Server */
  uint16_t SecurityWriteToServerCharHdl;
  uint16_t SecurityWriteToServerValueHdl;
  uint16_t SecurityWriteToServerDescHdl;

  /* handles of the Rx characteristic - Notification From Server */
  uint16_t SecurityNotificationCharHdl;
  uint16_t SecurityNotificationValueHdl;
  uint16_t SecurityNotificationDescHdl;
...
/* USER CODE BEGIN PV */
static uint8_t SecurityLedSelection;
static uint8_t SecurityLedLevel;

static SecurityButton_t SecurityButtonData;
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP */
static void Security_client_write_char(void);
/* USER CODE END PFP */
...
void GATT_CLIENT_APP_Init(void)
{
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_WRITE_CHAR_ID, UTIL_SEQ_RFU, Security_client_write_char);

  SecurityLedSelection = 0x00;
  SecurityLedLevel = 0x00;

  SecurityButtonData.selection = 0x01;
  SecurityButtonData.level = 0x00;
  ...
  else
  {
    ...
    switch (GattProcId)
    {
      ...
      case PROC_GATT_ENABLE_ALL_NOTIFICATIONS:
      {
        ...
        if(a_ClientContext[index].SecurityNotificationDescHdl != 0x0000)
        {
          result |= aci_gatt_clt_write(a_ClientContext[index].connHdl,
                                       BLE_GATT_UNENHANCED_ATT_L2CAP_CID,
                                       a_ClientContext[index].SecurityNotificationDescHdl,
                                       2,
                                       (uint8_t *) &enable);
          gatt_cmd_resp_wait();
...
__USED static void gatt_Notification(GATT_CLIENT_APP_Notification_evt_t *p_Notif)
{
  ...
  switch (p_Notif->Client_Evt_Opcode)
  {
    ...
    case SECURITY_NOTIFICATION_INFO_RECEIVED_EVT:
    {
      SecurityLedSelection = p_Notif->DataTransfered.p_Payload[0];
      UNUSED(SecurityLedSelection);
      SecurityLedLevel = p_Notif->DataTransfered.p_Payload[1];
      
      if (SecurityLedLevel == 0x00)
      {
        BSP_LED_Off(LED_BLUE);
        APP_DBG_MSG("  SECURITY APPLICATION CLIENT : NOTIFICATION RECEIVED - LED OFF \n");
      }
      else
      {
        BSP_LED_On(LED_BLUE);
        APP_DBG_MSG("  SECURITY APPLICATION CLIENT : NOTIFICATION RECEIVED - LED ON\n");
      }
      break;
    }
...
/* USER CODE BEGIN gatt_parse_services_1 */
      else if (uuid == ST_SECURITY_SERVICE_UUID)
      {
        a_ClientContext[index].SecurityServiceHdl = ServiceStartHdl;
        a_ClientContext[index].SecurityServiceEndHdl = ServiceEndHdl;

        APP_DBG_MSG(", SECURITY_SERVICE_UUID found\n");
      }
...
/* USER CODE BEGIN gatt_parse_chars_1 */
        else if (uuid == ST_SECURITY_NOTIFY_CHAR_UUID)
        {
          a_ClientContext[index].SecurityNotificationCharHdl = CharStartHdl;
          a_ClientContext[index].SecurityNotificationValueHdl = CharValueHdl;
          APP_DBG_MSG(", ST_SECURITY_NOTIFY_CHAR_UUID charac found\n");
        }
        else if (uuid == ST_SECURITY_WRITE_CHAR_UUID)
        {
          a_ClientContext[index].SecurityWriteToServerCharHdl = CharStartHdl;
          a_ClientContext[index].SecurityWriteToServerValueHdl = CharValueHdl;
          APP_DBG_MSG(", ST_SECURITY_WRITE_CHAR_UUID charac found\n");
        }
...
/* USER CODE BEGIN gatt_parse_descs_1 */
        else if (a_ClientContext[index].SecurityWriteToServerValueHdl == gattCharValueHdl)
        {
          a_ClientContext[index].SecurityWriteToServerDescHdl = handle;
          APP_DBG_MSG("SecurityWrite found : Desc UUID=0x%04X handle=0x%04X-0x%04X-0x%04X\n",
                      uuid, gattCharStartHdl, gattCharValueHdl, handle);
        }
        else if (a_ClientContext[index].SecurityNotificationValueHdl == gattCharValueHdl)
        {
          a_ClientContext[index].SecurityNotificationDescHdl = handle;
          APP_DBG_MSG("SecurityNotification found : Desc UUID=0x%04X handle=0x%04X-0x%04X-0x%04X\n",
                      uuid, gattCharStartHdl, gattCharValueHdl, handle);
        }
...
/* USER CODE BEGIN gatt_parse_descs_2 */
        ...
        {
          APP_DBG_MSG(", found ST_SECURITY_WRITE_CHAR_UUID\n");
        }
        else if (uuid == ST_SECURITY_NOTIFY_CHAR_UUID)
        {
          APP_DBG_MSG(", found ST_SECURITY_NOTIFY_CHAR_UUID\n");
        }
...
/* USER CODE BEGIN gatt_parse_notification_1 */
  ...
  if (a_ClientContext[index].connHdl == p_evt->Connection_Handle)
  {
    if (p_evt->Attribute_Handle == a_ClientContext[index].SecurityNotificationValueHdl)
    {
      APP_DBG_MSG("  Incoming Nofification received Security Server information\n");
      Notification.Client_Evt_Opcode = SECURITY_NOTIFICATION_INFO_RECEIVED_EVT;
...
static void Security_client_write_char(void)
{
  ...
  if (SecurityButtonData.level == 0x00)
  {
    SecurityButtonData.level = 0x01;
  }
  else
  {
    SecurityButtonData.level = 0x00;
  ...
  for(index = 0; index < BLE_CFG_MAX_NBR_GATT_EVT_HANDLERS; index++)
  {
    if (a_ClientContext[index].state != GATT_CLIENT_APP_IDLE)
    {
      ret = aci_gatt_clt_write_without_resp(a_ClientContext[index].connHdl,
                                            BLE_GATT_UNENHANCED_ATT_L2CAP_CID,
                                            a_ClientContext[index].SecurityWriteToServerValueHdl,
                                            2,
                                            (uint8_t *)&SecurityButtonData);
