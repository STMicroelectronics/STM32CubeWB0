...
static const char a_GapDeviceName[] = {  'S', 'e', 'r', 'i', 'a', 'l', ' ', 'P', 'o', 'r', 't', ' ', 'C', 'l', 'i', 'e', 'n', 't' }; /* Gap Device Name */
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
        if ((ad_length >= 7) && 
            ...
            (p_adv_data[i + 6] == FW_ID_SERIALPORT_SERVER))
        {
          /* Serial Port Server BlueST v2 detected */
...
static void Connect_Request(void)
{
  ...
  if (bleAppContext.deviceServerFound != 0)
  {
    ...
    result = aci_gap_set_connection_configuration(LE_1M_PHY_BIT,
                                                  CONN_INT_MS(20u), CONN_INT_MS(100u),
                                                  0u,
                                                  CONN_SUP_TIMEOUT_MS(1000u),
                                                  CONN_CE_LENGTH_MS(20u), CONN_CE_LENGTH_MS(20u));
