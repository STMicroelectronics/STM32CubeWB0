...
/**
  ...
  *          through the STM32WB0x UTILS LL API.
  ******************************************************************************
  * @attention
  *
...
void GetMCUInfo(void)
{
  ...
  sprintf((char *)aShowDeviceID, DEVICE_ID, LL_SYSCFG_GetDeviceID());

  /* Display Revision ID in string format */
  sprintf((char *)aShowRevisionID, REVISION_ID, LL_SYSCFG_GetDeviceRevision());
  /* Display UID Word0 */
