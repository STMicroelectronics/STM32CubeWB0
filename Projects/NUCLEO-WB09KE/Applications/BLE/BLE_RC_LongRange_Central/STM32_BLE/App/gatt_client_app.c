...
typedef struct
{
  ...
  /* For Central Role */
  uint16_t  Rc_peripheralSvcHdle;				/**< Rc_peripheral Service Handle */
  uint16_t  Rc_peripheralSvcEndHdle;				/**< Rc_peripheral Service End Handle */
...
/* USER CODE BEGIN gatt_parse_services_1 */
      else if (uuid == RC_LONGRANGE_UUID_16BIT)
      {
        a_ClientContext[index].Rc_peripheralSvcHdle = ServiceStartHdl;
        a_ClientContext[index].Rc_peripheralSvcEndHdle = ServiceEndHdl;

        APP_DBG_MSG(", RC_LONGRANGE_UUID_16BIT found\n");
      }
