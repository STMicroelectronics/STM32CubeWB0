...
typedef struct
{
  ...
  VTIMER_HandleType Write_Char_timer_Id;
  
  /* LED2 Timeout timerID. It is used to turn off LED2, 5ms after radio activity is received. */
  VTIMER_HandleType Scanning_Switch_On_Off_Led_timer_Id;

  /* Radio statistics. */
  VTIMER_HandleType Statistics_timer_Id;
...
static const char a_GapDeviceName[] = {  'R', 'C', '_', 'L', 'o', 'n', 'g', 'R', 'a', 'n', 'g', 'e', '_', 'C', 'e', 'n', 't', 'r', 'a', 'l' }; /* Gap Device Name */
...
static void Write_Char_Set_Task(void *arg);
void Stats_TimeoutCB(void *param);
...
/* USER CODE BEGIN Role_Mngt*/
  APP_DBG_MSG("\n\n * * * * * Application name : BLE_RC_LongRange_Central * * * * *\n\n");
...
void APP_BLE_Init(void)
{
  ...
  bleAppContext.Write_Char_timer_Id.callback = Write_Char_Set_Task;
  
  bleAppContext.Statistics_timer_Id.callback =  Stats_TimeoutCB;
  ...
  case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      if (p_disconnection_complete_event->Connection_Handle == bleAppContext.BleApplicationContext_legacy.connectionHandle)
      {
        ...
        HAL_RADIO_TIMER_StopVirtualTimer(&bleAppContext.Write_Char_timer_Id);
        HAL_RADIO_TIMER_StopVirtualTimer(&bleAppContext.Statistics_timer_Id);
        BSP_LED_Off(LD1);
        BSP_LED_Off(LD2);
        BSP_LED_Off(LD3);
      ...
      case HCI_LE_ADVERTISING_REPORT_SUBEVT_CODE:
        {
          ...
          /* HCI_LE_ADVERTISING_REPORT_SUBEVT_CODE Scanning */        
                                      ...
                                      uint16_t Supervision_Timeout)
{
  ...
  /* USER CODE BEGIN HCI_EVT_LE_CONN_COMPLETE */
   ...
   if (bleAppContext.Device_Connection_Status == APP_BLE_CONNECTED_CLIENT)
   {
     ...
     /* Connection as client, stop the Led2 Scanning blinking */
     HAL_RADIO_TIMER_StopVirtualTimer(&bleAppContext.Scanning_Switch_On_Off_Led_timer_Id);
     
     /* Once connected the LED2 is always on, on both devices Central and Peripheral. */
     BSP_LED_On(LD2);
     
     /* Start the timer to write to the Peripheral characteristic every 300ms. */
     HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.Write_Char_timer_Id, WRITE_CHAR_TIMER_MS);
     
     /* Start the timer to print the stats on the terminal. */
     HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.Statistics_timer_Id, STATS_INTERVAL_MS);  
   }
...
//    APP_DBG_MSG("p_address : 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
//                      p_address[5],
//                      p_address[4],
//                      p_address[3],
//                      p_address[2],
            ...
            (p_adv_data[i + 2] == ST_MANUF_ID_LSB)                  &&
              (p_adv_data[i + 3] == ST_MANUF_ID_MSB)                &&
                (p_adv_data[i + 4] == BLUESTSDK_V2)                 &&
                  (p_adv_data[i + 6] == FW_ID_RC_PERIPHERAL)        &&
                    (p_adv_data[i + 7] == FW_ID_RC_PERIPHERAL_D1)   &&
                      (p_adv_data[i + 8] == FW_ID_RC_PERIPHERAL_D2) &&
                        (p_adv_data[i + 9] == FW_ID_RC_PERIPHERAL_D3))
        {
...
static void Connect_Request(void)
{
  ...
  if (bleAppContext.deviceServerFound != 0)
  {
    ...
    result = aci_gap_set_connection_configuration(LE_CODED_PHY_BIT,
                                                  CONN_INT_MS(50u), 
                                                  CONN_INT_MS(100u),
                                                  0u,
                                                  CONN_SUP_TIMEOUT_MS(5000u),
    ...
    if (result == BLE_STATUS_SUCCESS)
    {
      APP_DBG_MSG("==>> (CODED PHY) aci_gap_set_connection_configuration Success , result: 0x%02X\n", result);
    }
    else
    {
      APP_DBG_MSG("==>> (CODED PHY) aci_gap_set_connection_configuration Failed , result: 0x%02X\n", result);
    }  
    
    if(phy == LE_1M_PHY_BIT)
    {
      APP_DBG_MSG("==>> (CODED PHY) phy = LE_1M_PHY_BIT\n");
    }
    else
    {
      APP_DBG_MSG("==>> (CODED PHY) phy = LE_CODED_PHY_BIT\n");
    }
    
    result = aci_gap_create_connection(phy,
                                      ...
                                      uint8_t RX_PHY)
{
  ...
  llc_conn_per_statistic(bleAppContext.BleApplicationContext_legacy.connectionHandle, &per_statistic_ptr);
  HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.Statistics_timer_Id, STATS_INTERVAL_MS);  
...
void Stats_TimeoutCB(void *param)
{
  ...
  APP_DBG_MSG("\n");
  APP_DBG_MSG("\tCRC errs        %2d%%\n", CRC_errs_perc);
  APP_DBG_MSG("\tMissed packets  %2d%%\n", missed_pckt_perc);
  APP_DBG_MSG("\tPER             %2d%%\n", packet_err_rate);
...
void APPE_Button1Action(void)
{
  ...
  if (bleAppContext.Device_Connection_Status == APP_BLE_CONNECTED_CLIENT )
  {
    APP_DBG_MSG("Device connected. Terminate the connection from the peripheral device.\n");
  }
...
void APPE_Button2Action(void)
{
  if(bleAppContext.Device_Connection_Status == APP_BLE_CONNECTED_CLIENT)
  {
    APP_DBG_MSG("  APP_BLE_Procedure_Gap_General(PROC_GAP_GEN_PHY_TOGGLE_CODED);\n");
...
void APPE_Button3Action(void)
{
  ...
  if(bleAppContext.Device_Connection_Status == APP_BLE_IDLE)
  {
    phy = LE_CODED_PHY_BIT;
    HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.Scanning_Switch_On_Off_Led_timer_Id, SCAN_SWITCH_ON_OFF_LED2_TIMEOUT_MS);
    APP_BLE_Procedure_Gap_Central(PROC_GAP_CENTRAL_SCAN_START_CODED_PHY);
  }
  return;
}
...
void APPE_Button1LongPressAction(void)
{
  ...
  {
    APP_DBG_MSG("  NO Action. bleAppContext.Device_Connection_Status   0x%02X\n", bleAppContext.Device_Connection_Status);
  }
  return;
}
...
void APPE_Button3LongPressAction(void)
{
  ...
  {
    APP_DBG_MSG("(CODED)  NO Action. bleAppContext.Device_Connection_Status   0x%02X\n", bleAppContext.Device_Connection_Status);
  }
  return;
}
