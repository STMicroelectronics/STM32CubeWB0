...
typedef struct
{
  ...
  VTIMER_HandleType TimerAdvLowPower_Id;
...
SCPS_APP_ConnHandleNotEvt_t SCPSHandleNotification;

static const char a_GapDeviceName[] = {  'H', 'I', 'D', ' ', 'M', 'o', 'u', 's', 'e' }; /* Gap Device Name */
...
uint8_t a_AdvData[] =
{
  ...
  7, AD_TYPE_COMPLETE_LOCAL_NAME, 'H', 'I', 'D', '_', 'X', 'X',  /* Complete name */
  3, AD_TYPE_16_BIT_SERV_UUID_CMPLT_LIST, 0x12, 0x18,
...
void APP_BLE_Init(void)
{
  ...
  */
  bleAppContext.TimerAdvLowPower_Id.callback = APP_BLE_AdvLowPower_timCB;
  HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.TimerAdvLowPower_Id, ADV_TIMEOUT_MS);
  ...
  case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      SCPSHandleNotification.EvtOpcode = SCPS_DISCON_HANDLE_EVT;
      SCPSHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      SCPS_APP_EvtRx(&SCPSHandleNotification);
      ...
      case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:
        {
          ...
          if (p_pairing_complete->Status == 0)
          {  
            devContext.deviceReadyToNotify = TRUE;
            APP_DBG_MSG("!!! Start virtual timer to update mouse coordinates!\n");
            startNotifyTimer();
          }
          else
            devContext.deviceReadyToNotify = FALSE;
                                      ...
                                      uint16_t Supervision_Timeout)
{
  ...
  HIDSHandleNotification.ConnectionHandle = Connection_Handle;
  HIDS_APP_EvtRx(&HIDSHandleNotification);
  ...
  BASHandleNotification.ConnectionHandle = Connection_Handle;
  BAS_APP_EvtRx(&BASHandleNotification);
  SCPSHandleNotification.EvtOpcode = SCPS_CONN_HANDLE_EVT;
  SCPSHandleNotification.ConnectionHandle = Connection_Handle;
  SCPS_APP_EvtRx(&SCPSHandleNotification);

  /* USER CODE BEGIN HCI_EVT_LE_CONN_COMPLETE */
  /* The connection is done, there is no need anymore to schedule the LP ADV */
  HAL_RADIO_TIMER_StopVirtualTimer(&(bleAppContext.TimerAdvLowPower_Id));
