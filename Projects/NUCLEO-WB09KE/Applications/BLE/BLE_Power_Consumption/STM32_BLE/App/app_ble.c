...
static const char a_GapDeviceName[] = {  'P', 'o', 'w', 'e', 'r', ' ', 'C', 'o', 'n', 's', 'u', 'm', 'p', 't', 'i', 'o', 'n' }; /* Gap Device Name */
...
uint8_t a_AdvData[] =
{
  ...
  21, AD_TYPE_COMPLETE_LOCAL_NAME, 'P', 'o', 'w', 'e', 'r', 'C', 'o', 'n', 's', 'u', 'm', 'p', 't', 'i', 'o', 'n', 'T', 'e', 's', 't',  /* Complete name */
};
...
static void  clkUsartState(uint8_t status);
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
  ...
  switch(ProcGapPeripheralId)
  {
    ...
    case PROC_GAP_PERIPH_ADVERTISE_START_LP:
    {
      ...
      /* Stop ADV procedure if already on going */
...
{ 
  /* Launch Fast advertising (100 ms) */
  if ((bleAppContext.Device_Connection_Status == APP_BLE_IDLE) ||
      (bleAppContext.Device_Connection_Status == APP_BLE_ADV_LP))
  {
#if (CFG_LPM_SUPPORTED == 1)
    clkUsartState(ENABLE);    
#endif /* CFG_LPM_SUPPORTED */

    APP_DBG_MSG("Fast ADV Start (100 ms)\n");
    APP_BLE_Procedure_Gap_Peripheral(PROC_GAP_PERIPH_ADVERTISE_START_FAST);
    bleAppContext.Device_Connection_Status = APP_BLE_ADV_FAST;
...
{
  /* Launch Slow advertising (1000 ms) */
  if ((bleAppContext.Device_Connection_Status == APP_BLE_IDLE) ||
      (bleAppContext.Device_Connection_Status == APP_BLE_ADV_FAST))
  {
#if (CFG_LPM_SUPPORTED == 1)
    ...
    APP_DBG_MSG("Slow ADV Start (1000 ms)\n");
...
void APPE_Button3Action(void)
{
  if (bleAppContext.Device_Connection_Status == APP_BLE_CONNECTED_SERVER)
  {
    ...
    APP_DBG_MSG("Disconnection Procedure Start\n");
    bleAppContext.Device_Connection_Status = APP_BLE_DISCONNECTING;
...
static void  clkUsartState(uint8_t status)
{
  if (status == ENABLE)
  {
    /* Enable USART CLOCK for the DEBUG MSG */
    __HAL_RCC_USART1_CLK_ENABLE();
  }
  else
  {
    /* Wait until the USART FIFO is empty */
    while ((__HAL_UART_GET_FLAG(&hcom_uart[COM1], UART_FLAG_TXFNF) == 0) ||
           (__HAL_UART_GET_FLAG(&hcom_uart[COM1], UART_FLAG_TC) == 0));
    /* Disable USART CLOCK to reduce the power consumption */
