...
void COC_PERIPH_APP_Notification(COC_APP_ConnHandle_Not_evt_t *pNotification)
{
  ...
  switch(pNotification->CoC_Evt_Opcode)
  {
    case EXCHANGE_ATT_MTU:
      ret = aci_gatt_clt_exchange_config(BleCoCContextPeriph.Conn_Handle);
    ...
    case BLE_CONN_HANDLE_EVT :
      UTIL_SEQ_SetTask( 1<<CFG_TASK_CONN_UPDATE_REG_ID, CFG_SEQ_PRIO_0);
      break;
...
void UartRxCpltCallback(uint8_t *pdata, uint16_t size)
{
  ...
  if(size == 1)
  {
    ...
    if( (byte_received == '\n') || (byte_received == '\r') || (buffUartRxIndex >= sizeof(a_buffUartRx)))
    {
...
void PeriphSendData( void )
{
  ...
  status = aci_l2cap_cos_sdu_data_transmit(BleCoCContextPeriph.Conn_Handle,
  ...
  {
    APP_DBG_MSG("==>> aci_l2cap_transmit_sdu_data : Fail, reason: 0x%02X\n", status);
  }
