...
_Important_: BLE_p2pServer_StaticStack_ota project by default uses ble_static_stack_sym.a from its Binary directory. After generating the new binary, make sure that its symbol table (generated in BLE_StaticStack_ota folder) is referenced by the application.
