...
#define SRVC_UUID               0xF2E0
#define TX_CHR_UUID             0xF2E1
#define RX_CHR_UUID             0xF2E2
...
void GATT_CLIENT_APP_Init(void)
{
  ...
  SerialPort_Client_APP_Terminal_Init();
  
  APP_DBG_MSG("-- SERIAL PORT CLIENT INITIALIZED \n");
...
uint8_t compare_array(uint8_t *X, uint8_t *Y, uint8_t dim)
{
  ...
  for(uint8_t i=0; i<dim && isEqual; i++)
  {
    if (X[i] != Y[i])
        isEqual=0;
  }
