...
static BLEEVT_EvtAckStatus_t CLIENT_EventHandler(aci_blecore_event *p_evt);
...
void GATT_CLIENT_APP_Init(void)
{
   ...
   */
  BLEEVT_RegisterGattEvtHandler(CLIENT_EventHandler);
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_WRITE_CHAR_ID, UTIL_SEQ_RFU, central_write_char);
...
void GATT_CLIENT_APP_Notification(GATT_CLIENT_APP_ConnHandle_Notif_evt_t *p_Notif)
{
  ...
  switch(p_Notif->ConnOpcode)
  {
      ...
      /* USER CODE BEGIN PEER_DISCON_HANDLE_EVT */
    {
      ...
      while((index < CFG_BLE_NUM_CLT_CONTEXT_MAX) &&
            (a_ClientContext[index].state != GATT_CLIENT_APP_IDLE))
      {
        a_ClientContext[index].state = GATT_CLIENT_APP_IDLE;
      }
