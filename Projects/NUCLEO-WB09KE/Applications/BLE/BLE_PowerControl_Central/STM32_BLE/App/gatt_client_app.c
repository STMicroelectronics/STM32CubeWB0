...
typedef enum
{
  ...
  APP_START_TIMER_EVT,
  APP_STOP_TIMER_EVT,
  APP_NOTIFICATION_INFO_RECEIVED_EVT,
...
typedef struct
{
  ...
  /* For Central Role */
  uint16_t  Pc_peripheralSvcHdle;				/**< Pc_peripheral Service Handle */
  uint16_t  Pc_peripheralSvcEndHdle;				/**< Pc_peripheral Service End Handle */
...
__USED static void gatt_Notification(GATT_CLIENT_APP_Notification_evt_t *p_Notif)
{
  ...
  switch (p_Notif->Client_Evt_Opcode)
  {
    ...
    case APP_NOTIFICATION_INFO_RECEIVED_EVT:
    {
      ...
      if (sLedLevel == 0x00)
      {
        APP_DBG_MSG("  APPLICATION CLIENT : NOTIFICATION RECEIVED \n");
      }
      else
      {
        APP_DBG_MSG("  APPLICATION CLIENT : NOTIFICATION RECEIVED\n");
...
/* USER CODE BEGIN gatt_parse_services_1 */
      else if (uuid == POWER_CONTROL_P_UUID_16BIT)
      {
        a_ClientContext[index].Pc_peripheralSvcHdle = ServiceStartHdl;
        a_ClientContext[index].Pc_peripheralSvcEndHdle = ServiceEndHdl;

        APP_DBG_MSG(", POWER_CONTROL_P_UUID_16BIT found\n");
      }
...
/* USER CODE BEGIN gatt_parse_notification_1 */
  ...
  if (a_ClientContext[index].connHdl == p_evt->Connection_Handle)
  {
    if (p_evt->Attribute_Handle == a_ClientContext[index].Switch_CValueHdle)
    {
      ...
      notification.Client_Evt_Opcode = APP_NOTIFICATION_INFO_RECEIVED_EVT;
