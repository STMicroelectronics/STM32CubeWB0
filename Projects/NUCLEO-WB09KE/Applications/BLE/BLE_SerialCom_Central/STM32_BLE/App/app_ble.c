  ...
  case HCI_VENDOR_EVT_CODE:
    {
      ...
      case ACI_GAP_PROC_COMPLETE_VSEVT_CODE:
        {
          ...
          /* USER CODE BEGIN EVT_GAP_PROCEDURE_COMPLETE */
                ...
                (p_gap_proc_complete->Status == 0x00))
            {
              ...
              UTIL_SEQ_SetTask(1 << CFG_TASK_CONN_DEV_1_ID, CFG_SEQ_PRIO_0);
      ...
      case ACI_L2CAP_COS_CONNECTION_RESP_VSEVT_CODE:
        {
          aci_l2cap_cos_connection_resp_event_rp0 *p_l2cap_cos_connection;
          p_l2cap_cos_connection = (aci_l2cap_cos_connection_resp_event_rp0 *) p_blecore_evt->data;
          ...
          APP_DBG_MSG("Channel_Type: %d\nPeer_MTU: %d\nPeer_MPS: %d\nInitial_Credits:%d\nResult: 0x%02X\nCID_Count: %d\n",
                      ...
                      p_l2cap_cos_connection->Initial_Credits, 
                      p_l2cap_cos_connection->Result,
                      p_l2cap_cos_connection->CID_Count);
          if(p_l2cap_cos_connection->CID_Count > 0)
          {
            
            BleCoCContextCentral.Conn_Handle = p_l2cap_cos_connection->Connection_Handle;
            BleCoCContextCentral.Peer_Max_Transmission_Unit = p_l2cap_cos_connection->Peer_MTU;
            BleCoCContextCentral.Channel_ID = p_l2cap_cos_connection->CID[0];
      ...
      case ACI_L2CAP_COS_SDU_DATA_RX_VSEVT_CODE:
        {
          ...
          HandleNotification.ConnectionHandle = p_l2cap_sdu_data_rx_event->Connection_Handle; //bleAppContext.BleApplicationContext_legacy.connectionHandle;
          HandleNotification.DataLength = p_l2cap_sdu_data_rx_event->SDU_Length;
          
          ret = aci_l2cap_cos_sdu_data_extract(p_l2cap_sdu_data_rx_event->Connection_Handle, 
                                               p_l2cap_sdu_data_rx_event->CID, 
                                               MAX_SDU_SIZE,
                                               HandleNotification.DataTable,
                                               &HandleNotification.DataLength);
...
void COC_CENTRAL_APP_Notification(COC_APP_ConnHandle_Not_evt_t *pNotification)
{
  ...
  case BLE_CONN_UPDATE_EVT:
    ...
    {
      BleCoCContextCentral.SPSM = 0x80;
    
      APP_DBG_MSG("==>> Start Connection Oriented Channel\n");
      ret = aci_l2cap_cos_connection_req(BleCoCContextCentral.Conn_Handle,
                                         BleCoCContextCentral.SPSM,
                                         MAX_SDU_SIZE,
                                         64,
                                         CFG_L2CAP_CHANNEL_TYPE,
                                         1);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_l2cap_cos_connection_req command, result: 0x%02X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_l2cap_cos_connection_req command\n");
      }
    }
    break;
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
            ...
            (p_adv_data[i + 4] == 0x02) /* blueST v2*/)
        {
          ...
          if(p_adv_data[i + 6] == CFG_DEV_ID_BLE_COC)
          {
            ...
            APP_DBG_MSG("  Serial Port Server, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
...
static void Connect_Request(void)
{
  ...
  if (bleAppContext.deviceServerFound != 0)
  {
    ...
    result = aci_gap_set_scan_configuration(DUPLICATE_FILTER_ENABLED, 
                                            0x00, //SCAN_ACCEPT_ALL, 
                                            LE_1M_PHY_BIT, 
...
static void Scan_Request(void)
{
  ...
  if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_CLIENT)
  {
    ...
    result = aci_gap_set_scan_configuration(DUPLICATE_FILTER_ENABLED, 0x00, LE_1M_PHY_BIT, HCI_SCAN_TYPE_PASSIVE,
...
{
  tBleStatus status;
  status = aci_gap_set_security(bleAppContext.BleApplicationContext_legacy.connectionHandle, GAP_SECURITY_LEVEL_2, 0x00);
  if (status != BLE_STATUS_SUCCESS)
  {
...
void APPE_Button2Action(void)
{
  ...
  else
  {
    ...
    UTIL_SEQ_SetTask(1 << CFG_TASK_CENTRAL_SECURITY_REQ_ID, CFG_SEQ_PRIO_0);
  }
