...
#error "Invalid CFG_BD_ADDRESS_TYPE"
  ...
  fill_advData(&a_AdvData[0], sizeof(a_AdvData), (uint8_t*)bd_address);
                                      ...
                                      uint16_t Supervision_Timeout)
{
  ...
  HRSHandleNotification.ConnectionHandle = Connection_Handle;
  HRS_APP_EvtRx(&HRSHandleNotification);
...
void APPE_Button2Action(void)
{
  ...
  {
    /* Security Request */
    ret = aci_gap_set_security(bleAppContext.BleApplicationContext_legacy.connectionHandle, GAP_SECURITY_LEVEL_3, 0); 
    
    if (ret != BLE_STATUS_SUCCESS)
    {
      APP_DBG_MSG("==>> aci_gap_set_security() Fail , result: %d \n", ret);
    }
    else
    {
      APP_DBG_MSG("===>> aci_gap_set_security - Success\n");
    }
  }
