...
static const char a_GapDeviceName[] = {  'D', 'a', 't', 'a', ' ', 'T', 'h', 'r', 'o', 'u', 'g', 'h', 'p', 'u', 't', ' ', 'C', 'l', 'i', 'e', 'n', 't' }; /* Gap Device Name */
...
/* USER CODE BEGIN EV */
extern DTC_Context_t DTC_Context;
/* USER CODE END EV */
...
static void Connect_Request(void)
{
  ...
  for (device_index = 0 ; device_index < DT_DEVICE_COUNT_MAX; device_index++)
  {
    if (bleAppContext.deviceServerFound != 0)
    {
      APP_DBG_MSG("Create connection to DTserver\n");
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
        if ((ad_length >= 7) && 
            ...
            (p_adv_data[i + 6] == FW_ID_DT_SERVER))
        {
          ...
          APP_DBG_MSG("  DT server, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
