...
_Important_: BLE_p2pServer_StaticStack project by default uses ble_static_stack_sym.a from Binary directory, which contains the symbol table needed to correctly link the application to the BLE_StaticStack.hex binary. If application does not use the precompiled BLE_StaticStack.hex in Binary folder, make sure to use the correct ble_static_stack_sym.a file.
