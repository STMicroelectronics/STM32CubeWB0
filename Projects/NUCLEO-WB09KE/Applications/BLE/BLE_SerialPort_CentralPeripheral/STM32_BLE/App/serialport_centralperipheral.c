...
/**
  ******************************************************************************
  * @file    SerialPort_CentralPeripheral.c
  * @author  MCD Application Team
  * @brief   SerialPort_CentralPeripheral definition.
  ******************************************************************************
  * @attention
  *
...
typedef struct{
  uint16_t  Serialport_centralperipheralSvcHdle;				/**< Serialport_centralperipheral Service Handle */
...
}SERIALPORT_CENTRALPERIPHERAL_Context_t;
...
static SERIALPORT_CENTRALPERIPHERAL_Context_t SERIALPORT_CENTRALPERIPHERAL_Context;
...
/*
 * UUIDs for SerialPort CentralPeripheral service
 */
#define SERIALPORT_CENTRALPERIPHERAL_UUID			0x66,0x9a,0x0c,0x20,0x00,0x08,0x96,0x9e,0xe2,0x11,0x9e,0xb1,0xe0,0xf2,0x73,0xd9
...
/* SerialPort CentralPeripheral service TX (notification) RX (write without response), characteristics definition */
static const ble_gatt_chr_def_t serialport_centralperipheral_chars[] = {
	{
...
/* SerialPort CentralPeripheral service definition */
static const ble_gatt_srv_def_t serialport_centralperipheral_service = {
   .type = BLE_GATT_SRV_PRIMARY_SRV_TYPE,
   .uuid = BLE_UUID_INIT_128(SERIALPORT_CENTRALPERIPHERAL_UUID),
   .chrs = {
       .chrs_p = (ble_gatt_chr_def_t *)serialport_centralperipheral_chars,
       .chr_count = 2U,
   },
};
 ...
 */
static BLEEVT_EvtAckStatus_t SERIALPORT_CENTRALPERIPHERAL_EventHandler(aci_blecore_event *p_evt)
{
  ...
  SERIALPORT_CENTRALPERIPHERAL_NotificationEvt_t notification;
  ...
  switch(p_evt->ecode)
  {
    case ACI_GATT_SRV_ATTRIBUTE_MODIFIED_VSEVT_CODE:
    {
      ...
      if(p_attribute_modified->Attr_Handle == (SERIALPORT_CENTRALPERIPHERAL_Context.TxCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
      {
        ...
        case (!BLE_GATT_SRV_CCCD_NOTIFICATION):
          ...
          notification.EvtOpcode = SERIALPORT_CENTRALPERIPHERAL_TX_NOTIFY_DISABLED_EVT;
          SERIALPORT_CENTRALPERIPHERAL_Notification(&notification);
        ...
        case BLE_GATT_SRV_CCCD_NOTIFICATION:
          ...
          notification.EvtOpcode = SERIALPORT_CENTRALPERIPHERAL_TX_NOTIFY_ENABLED_EVT;
          SERIALPORT_CENTRALPERIPHERAL_Notification(&notification);
      ...
      else if(p_attribute_modified->Attr_Handle == (SERIALPORT_CENTRALPERIPHERAL_Context.RxCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;

        notification.EvtOpcode = SERIALPORT_CENTRALPERIPHERAL_RX_WRITE_NO_RESP_EVT;
        ...
        SERIALPORT_CENTRALPERIPHERAL_Notification(&notification);
      } /* if(p_attribute_modified->Attr_Handle == (SERIALPORT_CENTRALPERIPHERAL_Context.RxCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
    ...
    case ACI_GATT_SRV_WRITE_VSEVT_CODE:
    {
      ...
      if(p_write->Attribute_Handle == (SERIALPORT_CENTRALPERIPHERAL_Context.RxCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;
  ...
  return(return_value);
}/* end SERIALPORT_CENTRALPERIPHERAL_EventHandler */
...
void SERIALPORT_CENTRALPERIPHERAL_Init(void)
{
  ...
  UNUSED(SERIALPORT_CENTRALPERIPHERAL_Context);
   ...
   */
  BLEEVT_RegisterGattEvtHandler(SERIALPORT_CENTRALPERIPHERAL_EventHandler);

  ret = aci_gatt_srv_add_service((ble_gatt_srv_def_t *)&serialport_centralperipheral_service);

  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_srv_add_service command: SerialPort_CentralPeripheral, error code: 0x%x \n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_srv_add_service command: SerialPort_CentralPeripheral \n");
  }

  SERIALPORT_CENTRALPERIPHERAL_Context.Serialport_centralperipheralSvcHdle = aci_gatt_srv_get_service_handle((ble_gatt_srv_def_t *) &serialport_centralperipheral_service);
  SERIALPORT_CENTRALPERIPHERAL_Context.TxCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&serialport_centralperipheral_chars[0]);
  SERIALPORT_CENTRALPERIPHERAL_Context.RxCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&serialport_centralperipheral_chars[1]);
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail registering SerialPort_CentralPeripheral handlers\n");
  }
...
tBleStatus SERIALPORT_CENTRALPERIPHERAL_UpdateValue(SERIALPORT_CENTRALPERIPHERAL_CharOpcode_t CharOpcode, SERIALPORT_CENTRALPERIPHERAL_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case SERIALPORT_CENTRALPERIPHERAL_RX:
...
tBleStatus SERIALPORT_CENTRALPERIPHERAL_NotifyValue(SERIALPORT_CENTRALPERIPHERAL_CharOpcode_t CharOpcode, SERIALPORT_CENTRALPERIPHERAL_Data_t *pData, uint16_t ConnectionHandle)
{
  ...
  switch(CharOpcode)
  {
    case SERIALPORT_CENTRALPERIPHERAL_TX:
      ret = aci_gatt_srv_notify(ConnectionHandle,
                                BLE_GATT_UNENHANCED_ATT_L2CAP_CID,
                                SERIALPORT_CENTRALPERIPHERAL_Context.TxCharHdle + 1,
                                GATT_NOTIFICATION,
