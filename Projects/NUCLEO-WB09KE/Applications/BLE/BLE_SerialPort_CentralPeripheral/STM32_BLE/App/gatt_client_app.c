...
typedef struct
{
  ...
  /* handles of the SerialPort service */
  uint16_t SerialPortServiceHdl;
  uint16_t SerialPortServiceEndHdl;

  /* handles of the Tx characteristic - Write To Server */
  uint16_t SerialPortWriteToServerCharHdl;
  uint16_t SerialPortWriteToServerValueHdl;
  uint16_t SerialPortWriteToServerDescHdl;

  /* handles of the Rx characteristic - Notification From Server */
  uint16_t SerialPortNotificationCharHdl;
  uint16_t SerialPortNotificationValueHdl;
  uint16_t SerialPortNotificationDescHdl;
...
/* Private defines ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define SERIALPORT_CENTRALPERIPHERAL_UUID__SHORT                           0xF2E0
#define TX_UUID__SHORT                                                     0xF2E1
#define RX_UUID__SHORT                                                     0xF2E2
...
static uint8_t a_buffSpTx[BUFFCOCTX_SIZE];
static uint8_t buffSpTxLen;
/* USER CODE END PV */
...
void GATT_CLIENT_APP_Init(void)
{
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_WRITE_RX_CHAR_ID, UTIL_SEQ_RFU, SerialPort_Client_Write_Char);

  APP_DBG_MSG("-- CLIENT INITIALIZED \n");
  ...
  switch(p_Notif->ConnOpcode)
  {
    ...
    case PEER_DISCON_HANDLE_EVT :
      /* USER CODE BEGIN PEER_DISCON_HANDLE_EVT */
      {
        ...
        while((index < GATT_CLT_MAX_NUM_SERVERS) && (a_ClientContext[index].state != GATT_CLIENT_APP_IDLE))
        {
      ...
      case PROC_GATT_ENABLE_ALL_NOTIFICATIONS:
      {
        ...
        if(a_ClientContext[index].SerialPortNotificationDescHdl != 0x0000)
        {
          result |= aci_gatt_clt_write(a_ClientContext[index].connHdl,
                                       ...
                                       (uint8_t *) &enable);
          gatt_cmd_resp_wait();
          APP_DBG_MSG(" SerialPortNotificationDescHdl =0x%04X\n",a_ClientContext[index].SerialPortNotificationDescHdl);
        }
...
/* USER CODE BEGIN gatt_parse_services_1 */
      else if (uuid == SERIALPORT_CENTRALPERIPHERAL_UUID__SHORT)
      { /* SERIALPORT_CENTRALPERIPHERAL_UUID */
        a_ClientContext[index].SerialPortServiceHdl = ServiceStartHdl;
        a_ClientContext[index].SerialPortServiceEndHdl = ServiceEndHdl;

        APP_DBG_MSG(", SERIALPORT_CENTRALPERIPHERAL_UUID found\n");
      }
...
/* USER CODE BEGIN gatt_parse_chars_1 */
          else if (uuid == TX_UUID__SHORT)
          {
            tx_handle = CharStartHdl;
            a_ClientContext[index].SerialPortNotificationCharHdl = CharStartHdl;
            a_ClientContext[index].SerialPortNotificationValueHdl = CharValueHdl;
            APP_DBG_MSG(", ST_SERIALPORT_NOTIFY_CHAR_UUID charac found\n");
          }
          else if (uuid == RX_UUID__SHORT)
          {
            rx_handle = CharStartHdl;
            a_ClientContext[index].SerialPortWriteToServerCharHdl = CharStartHdl;
            a_ClientContext[index].SerialPortWriteToServerValueHdl = CharValueHdl;
            APP_DBG_MSG(", ST_SERIALPORT_WRITE_CHAR_UUID charac found\n");
          } 
...
/* USER CODE BEGIN gatt_parse_descs_1 */
        else if (a_ClientContext[index].SerialPortWriteToServerValueHdl == gattCharValueHdl)
        {
          UNUSED(gattCharStartHdl);
          a_ClientContext[index].SerialPortWriteToServerDescHdl = handle;
          APP_DBG_MSG(", SerialPortWrite found : Desc UUID=0x%04X handle=0x%04X-0x%04X-0x%04X\n",
                      uuid, gattCharStartHdl, gattCharValueHdl, handle);
        }
        else if (a_ClientContext[index].SerialPortNotificationValueHdl == gattCharValueHdl)
        {
          UNUSED(gattCharStartHdl);
          a_ClientContext[index].SerialPortNotificationDescHdl = handle;
          APP_DBG_MSG(", SerialPortNotification found : Desc UUID=0x%04X handle=0x%04X-0x%04X-0x%04X\n",
                      uuid, gattCharStartHdl, gattCharValueHdl, handle);
        }
...
/* USER CODE BEGIN gatt_parse_descs_2 */
        ...
        {
          APP_DBG_MSG(", found TX_CHR_UUID\n");
        }
        else if (uuid == RX_UUID__SHORT)
        {
          APP_DBG_MSG(", found RX_CHR_UUID\n");
        }
...
/* USER CODE BEGIN gatt_parse_notification_1 */
  ...
  APP_DBG_MSG("gatt_parse_notification\n");

  attr_handle = p_evt->Attribute_Handle;
  if(attr_handle == tx_handle+1)
  {
    for(int i = 0; i < p_evt->Attribute_Value_Length; i++) 
        printf("%c", p_evt->Attribute_Value[i]);
  }
  else
  {
...
void SerialPort_Client_Write_Char__set_params(uint8_t* i_a_buffSpTx, uint8_t i_buffSpTxLen)
{
  buffSpTxLen = i_buffSpTxLen;
  memcpy(&a_buffSpTx[0], &i_a_buffSpTx[0], i_buffSpTxLen);
}
