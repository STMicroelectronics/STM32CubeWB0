...
typedef struct
{
  ...
  uint16_t  Mc_peripheralSvcEndHdle;                            /**< Mc_peripheral Service End Handle */
  
  uint16_t  Led_CCharHdle;                                      /**< LED_C Characteristic Handle */
  uint16_t  Led_CValueHdle;		                        /**< LED_C Value Handle */
  uint16_t  Led_CDescHdle;		                        /**< LED_C Descriptor Handle */
                                                                
  uint16_t  Switch_CCharHdle;		                        /**< SWITCH_C Characteristic Handle */
  uint16_t  Switch_CValueHdle;		                        /**< SWITCH_C Value Handle */
  uint16_t  Switch_CDescHdle;		                        /**< SWITCH_C Descriptor Handle */
...
static BleClientAppContext_t a_ClientContext[CFG_BLE_NUM_CLT_CONTEXT_MAX];

static uint16_t gattCharStartHdl = 0;
static uint16_t gattCharValueHdl = 0;

/* USER CODE BEGIN PV */
static uint8_t sLedSelection;
static uint8_t sLedLevel;

static StatusButton_t sButtonData;
/* USER CODE END PV */
...
static BLEEVT_EvtAckStatus_t ROUTER_EventHandler(aci_blecore_event *p_evt);
...
/* USER CODE BEGIN PFP */
static void central_write_char(void);
/* USER CODE END PFP */
...
void GATT_CLIENT_APP_Init(void)
{
  ...
  for(index = 0; index < CFG_BLE_NUM_CLT_CONTEXT_MAX; index++)
  {
   ...
   */
  ret = BLEEVT_RegisterGattEvtHandler(ROUTER_EventHandler);
  if (ret != 0)
  {
    Error_Handler();
  }
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_WRITE_ALL_CHAR_ID, UTIL_SEQ_RFU, central_write_char);

  sLedSelection = 0x00;
  sLedLevel = 0x00;

  sButtonData.selection = 0x01;
  sButtonData.level = 0x00;
  ...
  switch(p_Notif->ConnOpcode)
  {
    ...
    case PEER_DISCON_HANDLE_EVT :
      /* USER CODE BEGIN PEER_DISCON_HANDLE_EVT */
      {
        ...
        while((index < CFG_BLE_NUM_CLT_CONTEXT_MAX) && (a_ClientContext[index].state != GATT_CLIENT_APP_IDLE))
        {
          a_ClientContext[index].state = GATT_CLIENT_APP_IDLE;
        }
      }
...
uint8_t GATT_CLIENT_APP_Set_Conn_Handle(uint8_t index, uint16_t connHdl)
{
  ...
  if (index < CFG_BLE_NUM_CLT_CONTEXT_MAX)
  {
...
uint8_t GATT_CLIENT_APP_Clear_Conn_Handle(uint8_t index, uint16_t connHdl)
{
  uint8_t ret;
  ...
  else
  {
    ...
    switch (GattProcId)
    {
      ...
      case PROC_GATT_ENABLE_ALL_NOTIFICATIONS:
      {
        ...
        if(a_ClientContext[index].Switch_CDescHdle != 0x0000)
        {
          result |= aci_gatt_clt_write(a_ClientContext[index].connHdl,
                                       BLE_GATT_UNENHANCED_ATT_L2CAP_CID,
                                       a_ClientContext[index].Switch_CDescHdle,
                                       2,
                                       (uint8_t *) &enable);
          gatt_cmd_resp_wait();
          APP_DBG_MSG(" Switch_CDescHdle =0x%04X\n",a_ClientContext[index].Switch_CDescHdle);
        }
...
__USED static void gatt_Notification(GATT_CLIENT_APP_Notification_evt_t *p_Notif)
{
  ...
  switch (p_Notif->Client_Evt_Opcode)
  {
    ...
    {
      sLedSelection = p_Notif->DataTransfered.p_Payload[0];
      UNUSED(sLedSelection);
      sLedLevel = p_Notif->DataTransfered.p_Payload[1];
...
/* USER CODE BEGIN gatt_parse_services_1 */
      ...
      {
        a_ClientContext[index].Mc_peripheralSvcHdle = ServiceStartHdl;
        a_ClientContext[index].Mc_peripheralSvcEndHdle = ServiceEndHdl;
...
/* USER CODE BEGIN gatt_parse_chars_1 */
        else if (uuid == SWITCH_C_UUID_16BIT)
        {
          a_ClientContext[index].Switch_CCharHdle = CharStartHdl;
          a_ClientContext[index].Switch_CValueHdle = CharValueHdl;
          APP_DBG_MSG(", SWITCH_C_UUID_16BIT charac found\n");
        }
        else if (uuid == LED_C_UUID_16BIT)
        {
          a_ClientContext[index].Led_CCharHdle = CharStartHdl;
          a_ClientContext[index].Led_CValueHdle = CharValueHdl;
          APP_DBG_MSG(", LED_C_UUID_16BIT charac found\n");
        }
...
/* USER CODE BEGIN gatt_parse_descs_1 */
        else if (a_ClientContext[index].Led_CValueHdle == gattCharValueHdl)
        {
          a_ClientContext[index].Led_CDescHdle = handle;
          APP_DBG_MSG("P2PWrite found : Desc UUID=0x%04X handle=0x%04X-0x%04X-0x%04X\n",
                      uuid, gattCharStartHdl, gattCharValueHdl, handle);
        }
        else if (a_ClientContext[index].Switch_CValueHdle == gattCharValueHdl)
        {
          a_ClientContext[index].Switch_CDescHdle = handle;
          APP_DBG_MSG("Switch_CDescHdle found : Desc UUID=0x%04X handle=0x%04X-0x%04X-0x%04X\n",
                      uuid, gattCharStartHdl, gattCharValueHdl, handle);
        }
...
/* USER CODE BEGIN gatt_parse_descs_2 */
        ...
        {
          APP_DBG_MSG(", found LED_C_UUID_16BIT\n");
        }
        else if (uuid == SWITCH_C_UUID_16BIT)
        {
          APP_DBG_MSG(", found SWITCH_C_UUID_16BIT\n");
        }
...
/* USER CODE BEGIN gatt_parse_notification_1 */
  ...
  if (a_ClientContext[index].connHdl == p_evt->Connection_Handle)
  {
    ...
    {
      APP_DBG_MSG("  Incoming Nofification received Peripheral information\n");
...
static void client_discover_all(void)
{
  for(uint8_t index=0; index<CFG_BLE_NUM_CLT_CONTEXT_MAX; index++)
  {
    if(a_ClientContext[index].connHdl != 0xFFFF && a_ClientContext[index].state == GATT_CLIENT_APP_IDLE)
    {
...
static void central_write_char(void)
{
  /* Specific implementation for CFG_TASK_WRITE_ALL_CHAR_ID */
  
  uint8_t index = 0;
  tBleStatus ret = BLE_STATUS_INVALID_PARAMS;

  if (sButtonData.level == 0x00)
  {
    sButtonData.level = 0x01;
  }
  else
  {
    sButtonData.level = 0x00;
