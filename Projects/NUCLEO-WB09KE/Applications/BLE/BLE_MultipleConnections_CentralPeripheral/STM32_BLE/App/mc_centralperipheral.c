...
/**
  ******************************************************************************
  * @file    MC_CentralPeripheral.c
  * @author  MCD Application Team
  * @brief   MC_CentralPeripheral definition.
  ******************************************************************************
  * @attention
  *
...
typedef struct{
  uint16_t  Mc_centralperipheralSvcHdle;				/**< Mc_centralperipheral Service Handle */
...
}MC_CENTRALPERIPHERAL_Context_t;
...
static MC_CENTRALPERIPHERAL_Context_t MC_CENTRALPERIPHERAL_Context;
...
/*
 * UUIDs for MultiC CentralPeripheral service
 */
#define MC_CENTRALPERIPHERAL_UUID			0x8f,0xe5,0xb3,0xd5,0x2e,0x7f,0x4a,0x98,0x2a,0x48,0x7a,0xcc,0x30,0xf1,0x00,0x00
#define LED_C_UUID			0x19,0xed,0x82,0xae,0xed,0x21,0x4c,0x9d,0x41,0x45,0x22,0x8e,0x31,0xf1,0x00,0x00
#define SWITCH_C_UUID			0x19,0xed,0x82,0xae,0xed,0x21,0x4c,0x9d,0x41,0x45,0x22,0x8e,0x32,0xf1,0x00,0x00
...
/* MultiC CentralPeripheral service LED_C (write without response), SWITCH_C (notification) characteristics definition */
static const ble_gatt_chr_def_t mc_centralperipheral_chars[] = {
	{
...
/* MultiC CentralPeripheral service definition */
static const ble_gatt_srv_def_t mc_centralperipheral_service = {
   .type = BLE_GATT_SRV_PRIMARY_SRV_TYPE,
   .uuid = BLE_UUID_INIT_128(MC_CENTRALPERIPHERAL_UUID),
   .chrs = {
       .chrs_p = (ble_gatt_chr_def_t *)mc_centralperipheral_chars,
       .chr_count = 2U,
   },
};
 ...
 */
static BLEEVT_EvtAckStatus_t MC_CENTRALPERIPHERAL_EventHandler(aci_blecore_event *p_evt)
{
  ...
  MC_CENTRALPERIPHERAL_NotificationEvt_t notification;
  ...
  switch(p_evt->ecode)
  {
    case ACI_GATT_SRV_ATTRIBUTE_MODIFIED_VSEVT_CODE:
    {
      ...
      if(p_attribute_modified->Attr_Handle == (MC_CENTRALPERIPHERAL_Context.Switch_CCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
      {
        ...
        case (!BLE_GATT_SRV_CCCD_NOTIFICATION):
          ...
          notification.EvtOpcode = MC_CENTRALPERIPHERAL_SWITCH_C_NOTIFY_DISABLED_EVT;
          MC_CENTRALPERIPHERAL_Notification(&notification);
        ...
        case BLE_GATT_SRV_CCCD_NOTIFICATION:
          ...
          notification.EvtOpcode = MC_CENTRALPERIPHERAL_SWITCH_C_NOTIFY_ENABLED_EVT;
          MC_CENTRALPERIPHERAL_Notification(&notification);
      ...
      else if(p_attribute_modified->Attr_Handle == (MC_CENTRALPERIPHERAL_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;

        notification.EvtOpcode = MC_CENTRALPERIPHERAL_LED_C_WRITE_NO_RESP_EVT;
        /* USER CODE BEGIN Service1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
        APP_DBG_MSG("-- GATT : LED CONFIGURATION RECEIVED [notification.ConnectionHandle:0x%04X]\n", notification.ConnectionHandle);
        notification.DataTransfered.Length = p_attribute_modified->Attr_Data_Length;
        notification.DataTransfered.p_Payload = p_attribute_modified->Attr_Data;
        /* USER CODE END Service1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
        MC_CENTRALPERIPHERAL_Notification(&notification);
      } /* if(p_attribute_modified->Attr_Handle == (MC_CENTRALPERIPHERAL_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
    ...
    case ACI_GATT_SRV_READ_VSEVT_CODE :
    {
	  ...
	  if(p_read->Attribute_Handle == (MC_CENTRALPERIPHERAL_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
	  {
		return_value = BLEEVT_Ack;
    ...
    case ACI_GATT_SRV_WRITE_VSEVT_CODE:
    {
      ...
      if(p_write->Attribute_Handle == (MC_CENTRALPERIPHERAL_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;
  ...
  return(return_value);
}/* end MC_CENTRALPERIPHERAL_EventHandler */
...
void MC_CENTRALPERIPHERAL_Init(void)
{
  ...
  UNUSED(MC_CENTRALPERIPHERAL_Context);
   ...
   */
  BLEEVT_RegisterGattEvtHandler(MC_CENTRALPERIPHERAL_EventHandler);

  ret = aci_gatt_srv_add_service((ble_gatt_srv_def_t *)&mc_centralperipheral_service);

  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_srv_add_service command: MC_CentralPeripheral, error code: 0x%x \n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_srv_add_service command: MC_CentralPeripheral \n");
  }

  MC_CENTRALPERIPHERAL_Context.Mc_centralperipheralSvcHdle = aci_gatt_srv_get_service_handle((ble_gatt_srv_def_t *) &mc_centralperipheral_service);
  MC_CENTRALPERIPHERAL_Context.Led_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&mc_centralperipheral_chars[0]);
  MC_CENTRALPERIPHERAL_Context.Switch_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&mc_centralperipheral_chars[1]);
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail registering MC_CentralPeripheral handlers\n");
  }
...
tBleStatus MC_CENTRALPERIPHERAL_UpdateValue(MC_CENTRALPERIPHERAL_CharOpcode_t CharOpcode, MC_CENTRALPERIPHERAL_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case MC_CENTRALPERIPHERAL_LED_C:
...
tBleStatus MC_CENTRALPERIPHERAL_NotifyValue(MC_CENTRALPERIPHERAL_CharOpcode_t CharOpcode, MC_CENTRALPERIPHERAL_Data_t *pData, uint16_t ConnectionHandle)
{
  ...
  switch(CharOpcode)
  {
    ...
    case MC_CENTRALPERIPHERAL_SWITCH_C:
      ret = aci_gatt_srv_notify(ConnectionHandle,
                                BLE_GATT_UNENHANCED_ATT_L2CAP_CID,
                                MC_CENTRALPERIPHERAL_Context.Switch_CCharHdle + 1,
                                GATT_NOTIFICATION,
