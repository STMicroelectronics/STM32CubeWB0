...
static void Write_Char(void *arg);
static void Send_Notify(void *arg);
/* USER CODE END PFP */
...
void BLE_Init(void)
{
  ...
  */
  ...
  APP_DBG_MSG("\n* * * * * * Application name : BLE_MultipleConnections_CP * * * * * *\n");
...
void APP_BLE_Init(void)
{
  ...
  */
  ...
  bleAppContext.write_char_timer_Id.callback = Write_Char; 
      ...
      case ACI_GAP_PROC_COMPLETE_VSEVT_CODE:
        {
          ...
          {
            APP_DBG_MSG("-- GAP_AUTO_CONNECTION_ESTABLISHMENT_PROC completed  \n");
            UTIL_SEQ_SetTask(1U <<  CFG_TASK_CONNECTION_REQUEST, CFG_SEQ_PRIO_0);
          }
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
        ...
        p_adv_data[i+6] = FW_ID_MC_CENTRALPERIPHERAL; /* FW ID */
        p_adv_data[i+7] = FW_ID_MC_CENTRALPERIPHERAL_D1; /* FW data 1 */
        p_adv_data[i+8] = FW_ID_MC_CENTRALPERIPHERAL_D2; /* FW data 2 */
        p_adv_data[i+9] = FW_ID_MC_CENTRALPERIPHERAL_D3; /* FW data 3 */
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
                        ...
                        (p_adv_data[i + 9] == FW_ID_MC_PERIPHERAL_D3))
        {
          ...
          /* 
          BLE_MultipleConnections_
          Note: The BLE_MultipleConnections_Central device can connect with a BLE_MultipleConnections_CP device. 
          Similarly, a BLE_MultipleConnections_Peripheral device can connect with a BLE_MultipleConnections_CP device. 
          However, a BLE_MultipleConnections_Central device is not designed in this example to connect with a BLE_MultipleConnections_Peripheral device. */
...
{  
  ...
  {
    APP_DBG_MSG("No bonded device found - No Start_Auto_Connection started.\n");
    return;
  }

uint32_t status, paramA = SCAN_INT_MS(500), paramB = SCAN_WIN_MS(250);
...
{ 
  ...
  HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.send_notify_timer_Id, WRITE_CHAR_TIMEOUT_MS);
}
...
static void Send_Notify(void *arg)
{
  ...
  HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.write_char_timer_Id, SEND_NOTIFY_TIMEOUT_MS);
}
...
void APPE_Button1Action(void)
{
  ...
  {
    APP_DBG_MSG("  Maximum number of Central connections reached. Advertising started in non discoverable mode.\n");
    /* advertising in non discoverable mode */
    Start_Advertise_Non_Discovery_Mode();
    return;
  }
  
  /* If the device is currently advertising in non discoverable mode, stop this advertising to allow starting advertising in general mode. */
  if (bleAppContext.Device_Connection_Status_Peripheral == APP_BLE_ADV_FAST_NON_DISC_WITH_FILTER)
  {
    Adv_Cancel();
  }
