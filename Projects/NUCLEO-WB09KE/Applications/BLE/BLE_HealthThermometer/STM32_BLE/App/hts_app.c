...
void HTS_Notification(HTS_NotificationEvt_t *p_Notification)
{
  ...
  switch(p_Notification->EvtOpcode)
  {
    ...
    case HTS_TEMM_INDICATE_ENABLED_EVT:
      ...
      HTS_Temm_interval = DEFAULT_HTS_MEASUREMENT_INTERVAL;
      HAL_RADIO_TIMER_StartVirtualTimer(&HTS_APP_Context.TimerMeasurement_Id, HTS_Temm_interval); //DEFAULT_HTS_MEASUREMENT_INTERVAL);
    ...
    case HTS_INT_NOTIFY_ENABLED_EVT:
      /* USER CODE BEGIN Service2Char3_NOTIFY_ENABLED_EVT */
        ...
        HTS_Int_interval = DEFAULT_HTS_MEASUREMENT_INTERVAL;
        HAL_RADIO_TIMER_StartVirtualTimer(&HTS_APP_Context.TimerIntTemp_Id, HTS_Int_interval);  //DEFAULT_HTS_MEASUREMENT_INTERVAL); 
      ...
      /* USER CODE BEGIN Service2Char4_READ_EVT */
        ...
        if(p_Notification->RangeInterval != 0)
        {
          ...
          if(HTS_APP_Context.Mei_Indication_Status != Mei_INDICATION_OFF)
          {
            ...
            {
              HTS_Temm_interval = (HTS_APP_Context.MeasurementIntervalChar)*DEFAULT_HTS_MEASUREMENT_INTERVAL;
              HAL_RADIO_TIMER_StartVirtualTimer(&HTS_APP_Context.TimerMeasurement_Id, HTS_Temm_interval);
              HTS_APP_Context.Temm_Indication_Status = Temm_INDICATION_ON;
            }
            else
            {
    ...
    case HTS_MEI_INDICATE_ENABLED_EVT:
      /* USER CODE BEGIN Service2Char4_INDICATE_ENABLED_EVT */
        ...
        HTS_Mei_interval = 2*DEFAULT_HTS_MEASUREMENT_INTERVAL;
        HAL_RADIO_TIMER_StartVirtualTimer(&HTS_APP_Context.TimerMeasInt_Id, HTS_Mei_interval);  //2*DEFAULT_HTS_MEASUREMENT_INTERVAL);
...
void HTS_APP_EvtRx(HTS_APP_ConnHandleNotEvt_t *p_Notification)
{
  ...
  switch(p_Notification->EvtOpcode)
  {
    ...
    case HTS_CONN_HANDLE_EVT :
      HTS_APP_Context.ConnectionHandle = p_Notification->ConnectionHandle;
    ...
    case HTS_DISCON_HANDLE_EVT :
      HTS_APP_Context.ConnectionHandle = 0xFFFF;
...
void HTS_APP_Init(void)
{
  ...
  HTS_APP_Context.TemperatureMeasurementChar.TimeStamp.Year = 2023;
   ...
   */
  HTS_APP_Context.TimerMeasInt_Id.callback = HTS_APP_UpdateMeasurementInterval_timCb;
   ...
   */
  HTS_APP_Context.TimerMeasurement_Id.callback = HTS_APP_Measurements_timCb;
  HTS_APP_Context.Temm_Indication_Status = Temm_INDICATION_OFF;
...
static void HTS_APP_UpdateMeasurementInterval_timCb(void *arg)
{
  ...
  HAL_RADIO_TIMER_StartVirtualTimer(&HTS_APP_Context.TimerMeasInt_Id, HTS_Mei_interval);
  return;
}
...
__USED void HTS_Temm_SendIndication(void) /* Property Indication */
{
  ...
  if (indication_on_off != Temm_INDICATION_OFF && HTS_APP_Context.ConnectionHandle != 0xFFFF)
  {
    HTS_NotifyValue(HTS_TEMM, &hts_indication_data, HTS_APP_Context.ConnectionHandle);
  }
...
__USED void HTS_Int_SendNotification(void) /* Property Notification */
{
  ...
  if (notification_on_off != Int_NOTIFICATION_OFF && HTS_APP_Context.ConnectionHandle != 0xFFFF)
  {
    HTS_NotifyValue(HTS_INT, &hts_notification_data, HTS_APP_Context.ConnectionHandle);
  }
...
__USED void HTS_Mei_SendIndication(void) /* Property Indication */
{
  ...
  if (indication_on_off != Mei_INDICATION_OFF && HTS_APP_Context.ConnectionHandle != 0xFFFF)
  {
    HTS_NotifyValue(HTS_MEI, &hts_indication_data, HTS_APP_Context.ConnectionHandle);
  }
...
void HTS_APP_Measurement(void)
{
     ...
     (HTS_APP_Context.Temm_Indication_Status == Temm_INDICATION_ON))
  {
    ...
    {
      HAL_RADIO_TIMER_StopVirtualTimer(&HTS_APP_Context.TimerMeasurement_Id);
      HTS_Temm_interval = DEFAULT_HTS_MEASUREMENT_INTERVAL*10;
      HAL_RADIO_TIMER_StartVirtualTimer(&HTS_APP_Context.TimerMeasurement_Id, HTS_Temm_interval);   //DEFAULT_HTS_MEASUREMENT_INTERVAL*10);
      /**
       * Flags update
       */
      ...
      HTS_UpdateValue(HTS_TEMM, &msg_conf);
      ret = HTS_NotifyValue(HTS_TEMM, &msg_conf, HTS_APP_Context.ConnectionHandle);
      if(ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("HTS_NotifyValue fails\n");
      }
    }
  }
  else
...
static void HTS_APP_IntermediateTemperature(void)
{
  ...
  if(HTS_APP_Context.Int_Notification_Status == Int_NOTIFICATION_ON)
  {
    ...
    msg_conf.p_Payload = atm_value;
    msg_conf.Length = tm_char_length;
    HTS_UpdateValue(HTS_INT, &msg_conf);
    ret = HTS_NotifyValue(HTS_INT, &msg_conf, HTS_APP_Context.ConnectionHandle);
    if(ret != BLE_STATUS_SUCCESS)
    {
      APP_DBG_MSG("HTS_NotifyValue fails\n");
    }
...
static void HTS_APP_MeasurementInterval(void)
{
  ...
  HTS_UpdateValue(HTS_MEI, &msg_conf);
  ret = HTS_NotifyValue(HTS_MEI, &msg_conf, HTS_APP_Context.ConnectionHandle);
  if(ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("HTS_NotifyValue fails\n");
  }
