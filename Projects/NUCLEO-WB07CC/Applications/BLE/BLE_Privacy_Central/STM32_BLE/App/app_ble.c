...
static const char a_GapDeviceName[] = {  'P', 'r', 'i', 'v', 'a', 'c', 'y', ' ', 'C', 'e', 'n', 't', 'r', 'a', 'l' }; /* Gap Device Name */
...
static uint8_t analyse_adv_report(uint8_t adv_data_size, uint8_t *p_adv_data,
                                  uint8_t address_type, uint8_t *p_address, uint8_t evt_type);
static uint8_t Find_DeviceName(uint8_t data_length, uint8_t *data_value);
static void Connect_Request(void);
static void General_Connection_Establishment(void);
...
/* USER CODE BEGIN Role_Mngt*/
  ...
  APP_DBG_MSG("\n* * * * Application name : BLE_Privacy_Central * * * *\n");
...
void APP_BLE_Init(void)
{
  ...
  */
  ...
  UTIL_SEQ_RegTask( 1u << CFG_TASK_CONNECT_REQ, UTIL_SEQ_RFU, Connect_Request);
      ...
      case HCI_LE_EXTENDED_ADVERTISING_REPORT_SUBEVT_CODE:
        {
          ...
          found_status = analyse_adv_report(p_ext_adv_report->Extended_Advertising_Report.Data_Length, p_ext_adv_report->Extended_Advertising_Report.Data,
                                            ...
                                            p_ext_adv_report->Extended_Advertising_Report.Event_Type);
          if (found_status != 0)
          {
            Terminate_Scanning();
          }
      ...
      case ACI_GAP_PROC_COMPLETE_VSEVT_CODE:
        {
          ...
          if ((p_gap_proc_complete->Procedure_Code == GAP_GENERAL_CONNECTION_ESTABLISHMENT_PROC) &&
              (p_gap_proc_complete->Status == 0x00))
          {
            APP_DBG_MSG("-- GAP_GENERAL_CONNECTION_ESTABLISHMENT_PROC completed\n");
            UTIL_SEQ_SetTask(1u << CFG_TASK_CONNECT_REQ, CFG_SEQ_PRIO_0);
          }
...
/**
 * @brief Analyzes the advertising report to find a specific device.
 * 
 * This function analyzes the advertising report data to determine if a specific device is found.
 * If the device is found, it updates the device server address and type in the application context.
 * 
 ...
 * @param evt_type Type of the advertising event.
 * @return uint8_t Returns 1 if the device is found, 0 otherwise.
 */
static uint8_t analyse_adv_report(uint8_t adv_data_size, uint8_t *p_adv_data,
                                  uint8_t address_type, uint8_t *p_address, uint8_t evt_type)
{
  ...
  {
    /* scan response */
    if(HAL_IS_BIT_SET(evt_type, 0x001B) && Find_DeviceName(adv_data_size, p_adv_data))
    {
      APP_DBG_MSG("\n\n\n\nHCI_LE_EXTENDED_ADVERTISING_REPORT_SUBEVT_CODE \n");
      ...
      APP_DBG_MSG("  BLE_Privacy Peripheral  address_type 0x%08X\n", address_type);
      APP_DBG_MSG("  BLE_Privacy Peripheral  BdAddr       %02X:%02X:%02X:%02X:%02X:%02X\n\n\n\n",
...
/**
 * @brief Extracts the device name.
 * 
 * This function extracts the device name from the provided data and checks if it matches the expected name.
 * 
 * @param data_length Length of the data.
 * @param data_value Pointer to the data value.
 * @return uint8_t Returns TRUE if the local name found is the expected one, FALSE otherwise.
 */
static uint8_t Find_DeviceName(uint8_t data_length, uint8_t *data_value)
{
  uint8_t index = 0;
  ...
  while (index < data_length) {
    ...
    /* Check if field is complete local name and the length is the expected one for BLE NEW Chat  */
    if (data_value[index+1] == AD_TYPE_COMPLETE_LOCAL_NAME) { 
      /* check if found device name is the expected one: local_name */ 
      if (memcmp(&data_value[index+1], &peripheral_local_name[0], (uint32_t)(sizeof(peripheral_local_name)-1) ) == 0)
        return TRUE;
      else
        return FALSE;
    } else {
...
/**
 * @brief Initiates a connection request to a found device server.
 * 
 * This function is the specific implementation for CFG_TASK_CONNECT_REQ.
 * This function handles the connection request process to a device server if it has been found.
 * It attempts to create a connection and waits for the connection complete event.
 * 
 * @return void
 */
static void Connect_Request(void)
...
/**
 * @brief Establishes a general connection.
 *
 * This function configures the BLE device to scan for other devices, sets the connection parameters,
 * and starts the general connection establishment procedure.
 *
 * @param None
 * @retval None
 */
static void General_Connection_Establishment(void)
{
  ...
  {
    bleAppContext.Device_Connection_Status = (APP_BLE_ConnStatus_t)APP_BLE_GENERAL_CONNECTION_SCANNING;
    APP_DBG_MSG("==>> aci_gap_start_procedure - Success\n");
  }
}

/**
 * @brief Terminates the general connection establishment scanning procedure.
 *
 * This function stops the ongoing general connection establishment scanning procedure by calling 
...
static void Terminate_Scanning(void)
{
  ...
  {
    bleAppContext.Device_Connection_Status = (APP_BLE_ConnStatus_t)APP_BLE_GENERAL_CONNECTION_TERMINATE;
    APP_DBG_MSG("==>> aci_gap_terminate_gap_proc - Success\n");
  }
}
 ...
 * 
 * This function is the specific implementation for TASK_BUTTON_1. When the user presses Button 1 (Push1),
 * if the device is not in the `APP_BLE_CONNECTED_CLIENT` state, the system starts the scan procedures.
 * 
 * @return void
 */
void APPE_Button1Action(void)
{  
  /* When the user presses the push button Push1, if the state is not APP_BLE_CONNECTED_CLIENT, the system starts the scan procedures. */
  if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_CLIENT)
  {
    General_Connection_Establishment();
  }
  return;
}
...
/**
 * @brief Handles the action for Button 3 press.
 * 
 * This function is the specific implementation for TASK_BUTTON_3. It terminates the current BLE connection
