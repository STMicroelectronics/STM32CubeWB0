...
typedef struct
{
  ...
  /* handles of the Privacy service */
  uint16_t PrivacyServiceHdl;
  uint16_t PrivacyServiceEndHdl;
      ...
      case PROC_GATT_ENABLE_ALL_NOTIFICATIONS:
      {
        ...
        if(a_ClientContext[index].PrivacyNotificationDescHdl != 0x0000)
        {
          result |= aci_gatt_clt_write(a_ClientContext[index].connHdl,
                                       ...
                                       (uint8_t *) &enable);
          gatt_cmd_resp_wait();
          APP_DBG_MSG(" PrivacyNotificationDescHdl = 0x%04X\n",a_ClientContext[index].PrivacyNotificationDescHdl);
        }
...
__USED static void gatt_Notification(GATT_CLIENT_APP_Notification_evt_t *p_Notif)
{
  ...
  switch (p_Notif->Client_Evt_Opcode)
  {
    ...
    {      
      APP_DBG_MSG("  PRIVACY APPLICATION CLIENT : NOTIFICATION RECEIVED - p_Payload[0] = 0x%02X \n", p_Notif->DataTransfered.p_Payload[0]);
      break;
    }
...
/* USER CODE BEGIN gatt_parse_chars_1 */
        else if (uuid == CHAR_PRIVACY_UUID)
        {
          ...
          APP_DBG_MSG(", CHAR_PRIVACY_UUID charac found\n");
        }
...
/* USER CODE BEGIN gatt_parse_notification_1 */
  ...
  if (a_ClientContext[index].connHdl == p_evt->Connection_Handle)
  {
    ...
    {
      APP_DBG_MSG("  Incoming Nofification received Privacy Server information\n");
