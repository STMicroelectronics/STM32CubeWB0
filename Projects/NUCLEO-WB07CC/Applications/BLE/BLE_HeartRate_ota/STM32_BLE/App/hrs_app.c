...
void HRS_Notification(HRS_NotificationEvt_t *p_Notification)
{
  ...
  switch(p_Notification->EvtOpcode)
  {
    ...
    case HRS_HRME_NOTIFY_ENABLED_EVT:
      ...
      HAL_RADIO_TIMER_StopVirtualTimer(&(HRS_APP_Context.TimerMeasurement_Id));
      HAL_RADIO_TIMER_StartVirtualTimer(&(HRS_APP_Context.TimerMeasurement_Id), HRS_APP_MEASUREMENT_INTERVAL);
...
void HRS_APP_EvtRx(HRS_APP_ConnHandleNotEvt_t *p_Notification)
{
  ...
  switch(p_Notification->EvtOpcode)
  {
    ...
    case HRS_CONN_HANDLE_EVT :
      HRS_APP_Context.ConnectionHandle = p_Notification->ConnectionHandle;
    ...
    case HRS_DISCON_HANDLE_EVT :
      HRS_APP_Context.ConnectionHandle = 0xFFFF;
...
void HRS_APP_Init(void)
{
  ...
  HRS_APP_Context.TimerMeasurement_Id.callback = HRS_APP_Measurements_timCB;

  UTIL_SEQ_RegTask( 1U << CFG_TASK_MEAS_REQ_ID, UTIL_SEQ_RFU, HRS_APP_Measurements);
...
__USED void HRS_Hrme_SendNotification(void) /* Property Notification */
{
  ...
  if (notification_on_off != Hrme_NOTIFICATION_OFF && HRS_APP_Context.ConnectionHandle != 0xFFFF)
  {
    HRS_NotifyValue(HRS_HRME, &hrs_notification_data, HRS_APP_Context.ConnectionHandle);
  }
...
static void HRS_APP_Measurements(void)
{
  ...
  if (HAL_RNG_GenerateRandomNumber(&hrng, &measurement) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  HRS_UpdateValue(HRS_HRME, &msg_conf);
  ret = HRS_NotifyValue(HRS_HRME, &msg_conf, HRS_APP_Context.ConnectionHandle);
  if(ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("HRS_NotifyValue fails\n");
  }
