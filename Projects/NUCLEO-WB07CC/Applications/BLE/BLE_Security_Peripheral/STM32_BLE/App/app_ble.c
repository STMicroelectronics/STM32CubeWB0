...
#define MAX_NUM_BONDED_DEVICES          10U
/* USER CODE END PD */
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
...
SECURITY_PERIPHERAL_APP_ConnHandleNotEvt_t SECURITY_PERIPHERALHandleNotification;

static const char a_GapDeviceName[] = {  'S', 'e', 'c', 'u', 'r', 'i', 't', 'y', '_', 'P', 'e', 'r', 'i', 'p', 'h', 'e', 'r', 'a', 'l' }; /* Gap Device Name */
...
/* USER CODE BEGIN Role_Mngt*/
  ...
  APP_DBG_MSG("\n* * * * Application name : BLE_Security_Peripheral * * * *\n");
  ...
  case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      SECURITY_PERIPHERALHandleNotification.EvtOpcode = SECURITY_PERIPHERAL_DISCON_HANDLE_EVT;
      SECURITY_PERIPHERALHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      SECURITY_PERIPHERAL_APP_EvtRx(&SECURITY_PERIPHERALHandleNotification);
      /* USER CODE BEGIN EVT_DISCONN_COMPLETE */
      
       UTIL_SEQ_SetTask( 1U << CFG_TASK_TIMER_STOP_SEND_NOTIF_ID, CFG_SEQ_PRIO_0); 
                                      ...
                                      uint16_t Supervision_Timeout)
{
  ...
  SECURITY_PERIPHERALHandleNotification.EvtOpcode = SECURITY_PERIPHERAL_CONN_HANDLE_EVT;
  SECURITY_PERIPHERALHandleNotification.ConnectionHandle = Connection_Handle;
  SECURITY_PERIPHERAL_APP_EvtRx(&SECURITY_PERIPHERALHandleNotification);
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
        ...
        p_adv_data[i+6] = FW_ID_SECURITY_PERIPHERAL; /* FW ID */
        p_adv_data[i+7] = FW_DATA_SECURITY_PERIPHERAL_1; /* FW data 1 */
        p_adv_data[i+8] = FW_DATA_SECURITY_PERIPHERAL_2; /* FW data 2 */
        p_adv_data[i+9] = FW_DATA_SECURITY_PERIPHERAL_3; /* FW data 3 */
...
/**
 * @brief Handle the action for Button 1 press.
 *
 * If the device is in an idle state, it relaunches advertising and starts an advertising
 * timeout timer. If the device is connected as a BLE server, it initiates the procedure
 * to terminate the connection.
 *
 * @note This function is specific to TASK_BUTTON_1.
 */
...
/**
 * @brief Handle the action for Button 2 press.
 *
 * If the device is not connected as a BLE server, it attempts to clear the security
 * database.
 *
 * @note This function is specific to TASK_BUTTON_2.
 */
...
/**
 * @brief Handle the action for Button 3 press.
 *
 * If the device is connected as a BLE server, it initiates the procedure to update
 * the connection parameters.
 *
 * @note This function is specific to TASK_BUTTON_3.
 */
