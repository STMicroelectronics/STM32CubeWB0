...
typedef struct
{
  ...
  uint16_t P2PR_periph_connHdl;
  /* USER CODE BEGIN Service1_APP_Context_t */
  uint8_t P2PR_device_status[P2P_DEVICE_COUNT_MAX * 2];
  uint16_t P2PR_device_connHdl[P2P_DEVICE_COUNT_MAX * 2];
  uint8_t P2PR_device_bd_addr_type[P2P_DEVICE_COUNT_MAX * 2];
...
void P2PR_APP_EvtRx(P2PR_APP_ConnHandleNotEvt_t *p_Notification)
{
  ...
  switch(p_Notification->EvtOpcode)
  {
    ...
    case P2PR_CENTR_CONN_HANDLE_EVT :
    ...
    case P2PR_PERIPH_CONN_HANDLE_EVT :
      /* USER CODE BEGIN Service1_APP_PERIPH_CONN_HANDLE_EVT */
      P2PR_APP_Context.P2PR_periph_connHdl = p_Notification->ConnectionHandle;
    ...
    case P2PR_DISCON_HANDLE_EVT :
      ...
      if(i == (P2P_DEVICE_COUNT_MAX * 2))
      {
        P2PR_APP_Context.P2PR_periph_connHdl = 0xFFFF;
      }
...
void P2PR_APP_Init(void)
{
  ...
  for ( i = 0 ; i < (P2P_DEVICE_COUNT_MAX * 2) ; i++)
  {
    ...
    P2PR_APP_Context.P2PR_device_connHdl[i] = 0xFFFF;
...
uint8_t P2PR_analyseAdvReport(uint8_t adv_data_size, uint8_t *p_adv_data,
                              uint8_t address_type, uint8_t *p_address)
{
  ...
  if (found_status != 0)
  {
    P2PR_APP_Context.P2PR_device_bd_addr_type[dev_idx] = address_type;
    memcpy(P2PR_APP_Context.a_P2PR_device_bd_addr[dev_idx], p_address, BD_ADDR_SIZE);
...
__USED void P2PR_Notiffwd_SendNotification(void) /* Property Notification */
{
  ...
  if (notification_on_off != Notiffwd_NOTIFICATION_OFF && P2PR_APP_Context.P2PR_periph_connHdl != 0xFFFF)
  {
    P2PR_NotifyValue(P2PR_NOTIFFWD, &p2pr_notification_data, P2PR_APP_Context.P2PR_periph_connHdl);
  }
...
__USED void P2PR_Devinfo_SendNotification(void) /* Property Notification */
{
  ...
  if (notification_on_off != Devinfo_NOTIFICATION_OFF && P2PR_APP_Context.P2PR_periph_connHdl != 0xFFFF)
  {
    P2PR_NotifyValue(P2PR_DEVINFO, &p2pr_notification_data, P2PR_APP_Context.P2PR_periph_connHdl);
  }
  ...
  for (device_index = 0 ; device_index < (P2P_DEVICE_COUNT_MAX * 2) ; device_index++)
  {
    if (P2PR_APP_Context.P2PR_device_status[device_index] == P2PR_DEV_FOUND)
    {
      ...
      result = aci_gap_set_connection_configuration(LE_1M_PHY_BIT,
                                                    CONN_INT_MS(100u), CONN_INT_MS(200u),
                                                    0u,
      ...
      result = aci_gap_create_connection(LE_1M_PHY_BIT,
                                         P2PR_APP_Context.P2PR_device_bd_addr_type[device_index],
...
static uint8_t P2PR_notifDevInfo(uint8_t dev_idx)
{
  ...
  if (dev_idx < (P2P_DEVICE_COUNT_MAX * 2))
  {
    ...
    if(P2PR_APP_Context.P2PR_periph_connHdl != 0xFFFF)
    {
      P2PR_NotifyValue(P2PR_DEVINFO, &data, P2PR_APP_Context.P2PR_periph_connHdl);
    }
