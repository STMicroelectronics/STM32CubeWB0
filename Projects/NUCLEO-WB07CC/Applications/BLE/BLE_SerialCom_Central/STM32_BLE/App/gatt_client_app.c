...
static BLEEVT_EvtAckStatus_t P2P_CLIENT_EventHandler(aci_blecore_event *p_evt)
{
  ...
  case ACI_ATT_CLT_FIND_BY_TYPE_VALUE_RESP_VSEVT_CODE:
    ...
    /* USER CODE BEGIN ACI_ATT_EXCHANGE_MTU_RESP_VSEVT_CODE */
          tBleStatus result;
          result = hci_le_set_data_length(a_ClientContext[0].connHdl, MIN(exchange_mtu_resp->MTU + 4, 251), 2120);
          if (result != BLE_STATUS_SUCCESS)
          {
...
static void CentralSendData( void )
{
  ...
  status = aci_l2cap_cos_sdu_data_transmit(BleCoCContextCentral.Conn_Handle,
                                       BleCoCContextCentral.Channel_ID, 
                                       buffCocTxLen, 
                                       a_buffCocTx);
