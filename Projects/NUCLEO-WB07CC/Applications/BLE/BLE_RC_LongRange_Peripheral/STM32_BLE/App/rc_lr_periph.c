...
typedef struct{
  uint16_t  Rc_lr_periphSvcHdle;				/**< Rc_lr_periph Service Handle */
...
static RC_LR_PERIPH_Context_t RC_LR_PERIPH_Context;
...
/*
 * UUIDs for RC LongRange Peripheral service
 */
#define RC_LR_PERIPH_UUID			0x8f,0xe5,0xb3,0xd5,0x2e,0x7f,0x4a,0x98,0x2a,0x48,0x7a,0xcc,0x20,0xf2,0x00,0x00
#define LED_C_UUID			0x19,0xed,0x82,0xae,0xed,0x21,0x4c,0x9d,0x41,0x45,0x22,0x8e,0x21,0xf2,0x00,0x00
#define SWITCH_C_UUID			0x19,0xed,0x82,0xae,0xed,0x21,0x4c,0x9d,0x41,0x45,0x22,0x8e,0x22,0xf2,0x00,0x00
...
/* RC LongRange Peripheral service LED_C (write without response), SWITCH_C (notification) characteristics definition */
static const ble_gatt_chr_def_t rc_lr_periph_chars[] = {
	{
...
/* RC LongRange Peripheral service definition */
static const ble_gatt_srv_def_t rc_lr_periph_service = {
   .type = BLE_GATT_SRV_PRIMARY_SRV_TYPE,
   .uuid = BLE_UUID_INIT_128(RC_LR_PERIPH_UUID),
   .chrs = {
       .chrs_p = (ble_gatt_chr_def_t *)rc_lr_periph_chars,
       .chr_count = 2U,
   },
};
 ...
 */
static BLEEVT_EvtAckStatus_t RC_LR_PERIPH_EventHandler(aci_blecore_event *p_evt)
{
  ...
  RC_LR_PERIPH_NotificationEvt_t notification;
  ...
  switch(p_evt->ecode)
  {
    case ACI_GATT_SRV_ATTRIBUTE_MODIFIED_VSEVT_CODE:
    {
      ...
      if(p_attribute_modified->Attr_Handle == (RC_LR_PERIPH_Context.Switch_CCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
      {
        ...
        case (!BLE_GATT_SRV_CCCD_NOTIFICATION):
          ...
          notification.EvtOpcode = RC_LR_PERIPH_SWITCH_C_NOTIFY_DISABLED_EVT;
          RC_LR_PERIPH_Notification(&notification);
        ...
        case BLE_GATT_SRV_CCCD_NOTIFICATION:
          ...
          notification.EvtOpcode = RC_LR_PERIPH_SWITCH_C_NOTIFY_ENABLED_EVT;
          RC_LR_PERIPH_Notification(&notification);
      ...
      else if(p_attribute_modified->Attr_Handle == (RC_LR_PERIPH_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;

        notification.EvtOpcode = RC_LR_PERIPH_LED_C_WRITE_NO_RESP_EVT;
        ...
        RC_LR_PERIPH_Notification(&notification);
      } /* if(p_attribute_modified->Attr_Handle == (RC_LR_PERIPH_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
    ...
    case ACI_GATT_SRV_READ_VSEVT_CODE :
    {
	  ...
	  if(p_read->Attribute_Handle == (RC_LR_PERIPH_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
	  {
		return_value = BLEEVT_Ack;
    ...
    case ACI_GATT_SRV_WRITE_VSEVT_CODE:
    {
      ...
      if(p_write->Attribute_Handle == (RC_LR_PERIPH_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;
...
void RC_LR_PERIPH_Init(void)
{
   ...
   */
  BLEEVT_RegisterGattEvtHandler(RC_LR_PERIPH_EventHandler);

  ret = aci_gatt_srv_add_service((ble_gatt_srv_def_t *)&rc_lr_periph_service);

  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_srv_add_service command: RC_LR_Periph, error code: 0x%x \n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_srv_add_service command: RC_LR_Periph \n");
  }

  RC_LR_PERIPH_Context.Rc_lr_periphSvcHdle = aci_gatt_srv_get_service_handle((ble_gatt_srv_def_t *) &rc_lr_periph_service);
  RC_LR_PERIPH_Context.Led_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&rc_lr_periph_chars[0]);
  RC_LR_PERIPH_Context.Switch_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&rc_lr_periph_chars[1]);
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail registering RC_LR_Periph handlers\n");
  }
  ...
  switch(CharOpcode)
  {
    ...
    case RC_LR_PERIPH_SWITCH_C:
      ...
      ret = aci_gatt_srv_notify(ConnectionHandle,
                                ...
                                RC_LR_PERIPH_Context.Switch_CCharHdle + 1,
                                GATT_NOTIFICATION,
