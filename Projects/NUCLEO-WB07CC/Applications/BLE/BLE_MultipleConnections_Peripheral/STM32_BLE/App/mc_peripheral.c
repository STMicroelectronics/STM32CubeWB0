...
/**
  ******************************************************************************
  * @file    MC_Peripheral.c
  * @author  MCD Application Team
  * @brief   MC_Peripheral definition.
  ******************************************************************************
  * @attention
  *
...
typedef struct{
  uint16_t  Mc_peripheralSvcHdle;				/**< Mc_peripheral Service Handle */
...
static MC_PERIPHERAL_Context_t MC_PERIPHERAL_Context;
...
/*
 * UUIDs for MultiC Peripheral service
 */
#define MC_PERIPHERAL_UUID			0x8f,0xe5,0xb3,0xd5,0x2e,0x7f,0x4a,0x98,0x2a,0x48,0x7a,0xcc,0x30,0xf1,0x00,0x00
...
/* MultiC Peripheral service LED_C (write without response), SWITCH_C (notification) characteristics definition */
static const ble_gatt_chr_def_t mc_peripheral_chars[] = {
	{
...
/* MultiC Peripheral service definition */
static const ble_gatt_srv_def_t mc_peripheral_service = {
   .type = BLE_GATT_SRV_PRIMARY_SRV_TYPE,
   .uuid = BLE_UUID_INIT_128(MC_PERIPHERAL_UUID),
   .chrs = {
       .chrs_p = (ble_gatt_chr_def_t *)mc_peripheral_chars,
       .chr_count = 2U,
   },
};
 ...
 */
static BLEEVT_EvtAckStatus_t MC_PERIPHERAL_EventHandler(aci_blecore_event *p_evt)
{
  ...
  MC_PERIPHERAL_NotificationEvt_t notification;
  ...
  switch(p_evt->ecode)
  {
    case ACI_GATT_SRV_ATTRIBUTE_MODIFIED_VSEVT_CODE:
    {
      ...
      if(p_attribute_modified->Attr_Handle == (MC_PERIPHERAL_Context.Switch_CCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
      {
        ...
        case (!BLE_GATT_SRV_CCCD_NOTIFICATION):
          ...
          notification.EvtOpcode = MC_PERIPHERAL_SWITCH_C_NOTIFY_DISABLED_EVT;
          MC_PERIPHERAL_Notification(&notification);
        ...
        case BLE_GATT_SRV_CCCD_NOTIFICATION:
          ...
          notification.EvtOpcode = MC_PERIPHERAL_SWITCH_C_NOTIFY_ENABLED_EVT;
          MC_PERIPHERAL_Notification(&notification);
      ...
      else if(p_attribute_modified->Attr_Handle == (MC_PERIPHERAL_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;

        notification.EvtOpcode = MC_PERIPHERAL_LED_C_WRITE_NO_RESP_EVT;
        ...
        MC_PERIPHERAL_Notification(&notification);
      } /* if(p_attribute_modified->Attr_Handle == (MC_PERIPHERAL_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
    ...
    case ACI_GATT_SRV_READ_VSEVT_CODE :
    {
	  ...
	  if(p_read->Attribute_Handle == (MC_PERIPHERAL_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
	  {
		return_value = BLEEVT_Ack;
    ...
    case ACI_GATT_SRV_WRITE_VSEVT_CODE:
    {
      ...
      if(p_write->Attribute_Handle == (MC_PERIPHERAL_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;
  ...
  return(return_value);
}/* end MC_PERIPHERAL_EventHandler */
...
void MC_PERIPHERAL_Init(void)
{
   ...
   */
  BLEEVT_RegisterGattEvtHandler(MC_PERIPHERAL_EventHandler);

  ret = aci_gatt_srv_add_service((ble_gatt_srv_def_t *)&mc_peripheral_service);

  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_srv_add_service command: MC_Peripheral, error code: 0x%x \n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_srv_add_service command: MC_Peripheral \n");
  }

  MC_PERIPHERAL_Context.Mc_peripheralSvcHdle = aci_gatt_srv_get_service_handle((ble_gatt_srv_def_t *) &mc_peripheral_service);
  MC_PERIPHERAL_Context.Led_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&mc_peripheral_chars[0]);
  MC_PERIPHERAL_Context.Switch_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&mc_peripheral_chars[1]);
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail registering MC_Peripheral handlers\n");
  }
  ...
  switch(CharOpcode)
  {
    ...
    case MC_PERIPHERAL_SWITCH_C:
      ...
      ret = aci_gatt_srv_notify(ConnectionHandle,
                                ...
                                MC_PERIPHERAL_Context.Switch_CCharHdle + 1,
                                GATT_NOTIFICATION,
