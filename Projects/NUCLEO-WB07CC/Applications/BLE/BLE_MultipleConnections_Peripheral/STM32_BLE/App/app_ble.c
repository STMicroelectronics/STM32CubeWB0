...
* It terminates the current BLE connection with the error message BLE_ERROR_TERMINATED_LOCAL_HOST.
*
* @return void
*/
static void Terminate_Connection(void)
...
* 
* This function is called when B1 is pressed. If the device is idle, it starts advertising
* to accept a connection and starts a timer to stop advertising after a specified timeout.
* If the device is connected as a server, it sets a task to send a notification.
* 
* @retval None
*/
...
* 
* This function is called when Button 2 (B2) is pressed. If the device is connected as a BLE server,
* it sets a new task to send a notification to the already connected central device. Otherwise, it
...
void APPE_Button2Action(void)
{
  ...
  if (bleAppContext.Device_Connection_Status == APP_BLE_CONNECTED_SERVER)
  {
    /* Set a task to send a notification to the central device */
