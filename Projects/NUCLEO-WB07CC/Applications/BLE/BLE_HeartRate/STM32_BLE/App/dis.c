...
#define SNS_SIZE        32	/* Serial Number Characteristic size */
#define HRS_SIZE        32	/* Hardware Revision Characteristic size */
...
uint8_t sns_val_buffer[SNS_SIZE];

static ble_gatt_val_buffer_def_t sns_val_buffer_def = {
  .op_flags = BLE_GATT_SRV_OP_MODIFIED_EVT_ENABLE_FLAG | BLE_GATT_SRV_OP_VALUE_VAR_LENGTH_FLAG,
  .val_len = SNS_SIZE,
  .buffer_len = sizeof(sns_val_buffer),
  .buffer_p = sns_val_buffer
};

uint8_t hrs_val_buffer[HRS_SIZE];

static ble_gatt_val_buffer_def_t hrs_val_buffer_def = {
  .op_flags = BLE_GATT_SRV_OP_MODIFIED_EVT_ENABLE_FLAG | BLE_GATT_SRV_OP_VALUE_VAR_LENGTH_FLAG,
  .val_len = HRS_SIZE,
  .buffer_len = sizeof(hrs_val_buffer),
  .buffer_p = hrs_val_buffer
};
...
static const ble_gatt_chr_def_t dis_chars[] = {
    ...
    },
	{
        ...
        .val_buffer_p = &sns_val_buffer_def
    },
	{
        .properties = BLE_GATT_SRV_CHAR_PROP_READ,
        .permissions = BLE_GATT_SRV_PERM_NONE,
        .min_key_size = 0x10,
        .uuid = BLE_UUID_INIT_16(HARDWARE_REVISION_UUID),
        .val_buffer_p = &hrs_val_buffer_def
    },
	{
        .properties = BLE_GATT_SRV_CHAR_PROP_READ,
        .permissions = BLE_GATT_SRV_PERM_NONE,
...
static BLEEVT_EvtAckStatus_t DIS_EventHandler(aci_blecore_event *p_evt)
{
  ...
  switch(p_evt->ecode)
  {
    ...
    case ACI_GATT_SRV_READ_VSEVT_CODE :
    {
	  ...
	  else if(p_read->Attribute_Handle == (DIS_Context.SnsCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
	  {
		return_value = BLEEVT_Ack;
	  ...
	  } /* if(p_read->Attribute_Handle == (DIS_Context.SnsCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
	  else if(p_read->Attribute_Handle == (DIS_Context.HrsCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
	  {
		return_value = BLEEVT_Ack;
	  ...
	  } /* if(p_read->Attribute_Handle == (DIS_Context.HrsCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void DIS_Init(void)
{
  ...
  DIS_Context.SnsCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&dis_chars[2]);
  DIS_Context.HrsCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&dis_chars[3]);
  DIS_Context.FrsCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&dis_chars[4]);
...
tBleStatus DIS_UpdateValue(DIS_CharOpcode_t CharOpcode, DIS_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    ...
    case DIS_SNS:
      memcpy(sns_val_buffer, pData->p_Payload, MIN(pData->Length, sizeof(sns_val_buffer)));
    ...
    case DIS_HRS:
      memcpy(hrs_val_buffer, pData->p_Payload, MIN(pData->Length, sizeof(hrs_val_buffer)));
