...
POWERCONTROL_P_APP_ConnHandleNotEvt_t POWERCONTROL_PHandleNotification;

static const char a_GapDeviceName[] = {  'P', 'o', 'w', 'e', 'r', 'C', 'o', 'n', 't', 'r', 'o', 'l', '_', 'P', 'e', 'r', 'i', 'p', 'h', 'e', 'r', 'a', 'l' }; /* Gap Device Name */
...
uint8_t a_AdvData[] =
{
  ...
  10, AD_TYPE_COMPLETE_LOCAL_NAME, 'P', 'o', 'w', 'e', 'r', 'C', '_', 'X', 'X',  /* Complete name */
  ...
  case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      POWERCONTROL_PHandleNotification.EvtOpcode = POWERCONTROL_P_DISCON_HANDLE_EVT;
      POWERCONTROL_PHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      POWERCONTROL_P_APP_EvtRx(&POWERCONTROL_PHandleNotification);
      /* USER CODE BEGIN EVT_DISCONN_COMPLETE */
      /* At any disconnection, central and peripheral will reconnect automatically */
      APP_DBG_MSG(" Advertising start after disconnection.\n");
      if(phy == HCI_PHY_LE_1M)
      {
        APP_BLE_Procedure_Gap_Peripheral(PROC_GAP_PERIPH_ADVERTISE_START_FAST);
      }
      else
      {
        if(phy == HCI_PHY_LE_CODED)
        {
                                      ...
                                      uint16_t Supervision_Timeout)
{
  ...
  POWERCONTROL_PHandleNotification.EvtOpcode = POWERCONTROL_P_CONN_HANDLE_EVT;
  POWERCONTROL_PHandleNotification.ConnectionHandle = Connection_Handle;
  POWERCONTROL_P_APP_EvtRx(&POWERCONTROL_PHandleNotification);
  ...
  case PROC_GAP_PERIPH_ADVERTISE_START_CODED_PHY:
    {
      ...
      status = aci_gap_set_advertising_configuration(coded_phy_handle,
                                                     ...
                                                     HCI_ADV_FILTER_NONE,
                                                     0, /* 0 dBm */
                                                     HCI_PHY_LE_CODED, /* Primary advertising PHY */
                                                     0, /* 0 skips */
                                                     HCI_PHY_LE_CODED, /* Secondary advertising PHY. Not used with legacy advertising. */
                                                     0, /* SID */
                                                     0 /* No scan request notifications */);
      if (status != BLE_STATUS_SUCCESS)
      {
