...
/**
  ******************************************************************************
  * @file    PowerControl_P.c
  * @author  MCD Application Team
  * @brief   PowerControl_P definition.
  ******************************************************************************
  * @attention
  *
...
typedef struct{
  uint16_t  Powercontrol_pSvcHdle;				/**< Powercontrol_p Service Handle */
...
static POWERCONTROL_P_Context_t POWERCONTROL_P_Context;
...
/*
 * UUIDs for PowerControl Peripheral service
 */
#define POWERCONTROL_P_UUID			0x8f,0xe5,0xb3,0xd5,0x2e,0x7f,0x4a,0x98,0x2a,0x48,0x7a,0xcc,0x30,0xf3,0x00,0x00
#define LED_C_UUID			0x19,0xed,0x82,0xae,0xed,0x21,0x4c,0x9d,0x41,0x45,0x22,0x8e,0x31,0xf3,0x00,0x00
#define SWITCH_C_UUID			0x19,0xed,0x82,0xae,0xed,0x21,0x4c,0x9d,0x41,0x45,0x22,0x8e,0x32,0xf3,0x00,0x00
...
/* PowerControl Peripheral service LED_C (write without response), SWITCH_C (notification) characteristics definition */
static const ble_gatt_chr_def_t powercontrol_p_chars[] = {
	{
...
/* PowerControl Peripheral service definition */
static const ble_gatt_srv_def_t powercontrol_p_service = {
   .type = BLE_GATT_SRV_PRIMARY_SRV_TYPE,
   .uuid = BLE_UUID_INIT_128(POWERCONTROL_P_UUID),
   .chrs = {
       .chrs_p = (ble_gatt_chr_def_t *)powercontrol_p_chars,
       .chr_count = 2U,
   },
};
 ...
 */
static BLEEVT_EvtAckStatus_t POWERCONTROL_P_EventHandler(aci_blecore_event *p_evt)
{
  ...
  POWERCONTROL_P_NotificationEvt_t notification;
  ...
  switch(p_evt->ecode)
  {
    case ACI_GATT_SRV_ATTRIBUTE_MODIFIED_VSEVT_CODE:
    {
      ...
      if(p_attribute_modified->Attr_Handle == (POWERCONTROL_P_Context.Switch_CCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
      {
        ...
        case (!BLE_GATT_SRV_CCCD_NOTIFICATION):
          ...
          notification.EvtOpcode = POWERCONTROL_P_SWITCH_C_NOTIFY_DISABLED_EVT;
          POWERCONTROL_P_Notification(&notification);
        ...
        case BLE_GATT_SRV_CCCD_NOTIFICATION:
          ...
          notification.EvtOpcode = POWERCONTROL_P_SWITCH_C_NOTIFY_ENABLED_EVT;
          POWERCONTROL_P_Notification(&notification);
      ...
      else if(p_attribute_modified->Attr_Handle == (POWERCONTROL_P_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;

        notification.EvtOpcode = POWERCONTROL_P_LED_C_WRITE_NO_RESP_EVT;
        /* USER CODE BEGIN Service1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
        // APP_DBG_MSG("-- GATT : LED CONFIGURATION RECEIVED [ConnHandle:0x%04X]\n", notification.ConnectionHandle);
        notification.DataTransfered.Length = p_attribute_modified->Attr_Data_Length;
        notification.DataTransfered.p_Payload = p_attribute_modified->Attr_Data;
        /* USER CODE END Service1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
        POWERCONTROL_P_Notification(&notification);
      } /* if(p_attribute_modified->Attr_Handle == (POWERCONTROL_P_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
    ...
    case ACI_GATT_SRV_READ_VSEVT_CODE :
    {
	  ...
	  if(p_read->Attribute_Handle == (POWERCONTROL_P_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
	  {
		return_value = BLEEVT_Ack;
    ...
    case ACI_GATT_SRV_WRITE_VSEVT_CODE:
    {
      ...
      if(p_write->Attribute_Handle == (POWERCONTROL_P_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;
  ...
  return(return_value);
}/* end POWERCONTROL_P_EventHandler */
...
void POWERCONTROL_P_Init(void)
{
   ...
   */
  BLEEVT_RegisterGattEvtHandler(POWERCONTROL_P_EventHandler);

  ret = aci_gatt_srv_add_service((ble_gatt_srv_def_t *)&powercontrol_p_service);

  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_srv_add_service command: PowerControl_P, error code: 0x%x \n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_srv_add_service command: PowerControl_P \n");
  }

  POWERCONTROL_P_Context.Powercontrol_pSvcHdle = aci_gatt_srv_get_service_handle((ble_gatt_srv_def_t *) &powercontrol_p_service);
  POWERCONTROL_P_Context.Led_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&powercontrol_p_chars[0]);
  POWERCONTROL_P_Context.Switch_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&powercontrol_p_chars[1]);
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail registering PowerControl_P handlers\n");
  }
  ...
  switch(CharOpcode)
  {
    ...
    case POWERCONTROL_P_SWITCH_C:
      ...
      ret = aci_gatt_srv_notify(ConnectionHandle,
                                ...
                                POWERCONTROL_P_Context.Switch_CCharHdle + 1,
                                GATT_NOTIFICATION,
