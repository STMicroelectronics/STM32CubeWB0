...
static const char a_GapDeviceName[] = {  'S', 'e', 'r', 'i', 'a', 'l', ' ', 'C', 'o', 'm', ' ', 'P', 'e', 'r', 'i', 'p', 'h', 'e', 'r', 'a', 'l' }; /* Gap Device Name */
...
/* USER CODE BEGIN PV */
BleCoCContext_t BleCoCContextPeriph;
  ...
  case HCI_VENDOR_EVT_CODE:
    {
      ...
      case ACI_L2CAP_COS_CONNECTION_REQ_VSEVT_CODE:
        {
          ...
          tBleStatus ret;
          uint16_t result = L2CAP_CONN_SUCCESSFUL;
          ...
          if(p_l2cap_cos_connection->CID_Count > 1)
          {
            result = L2CAP_CONN_FAIL_INSUFFICIENT_RESOURCES;
          }
          if(p_l2cap_cos_connection->Channel_Type == L2CAP_CHANNEL_TYPE_LE_CFC)
          {
            ret = aci_l2cap_cos_connection_resp(p_l2cap_cos_connection->Connection_Handle,
                                                ...
                                                result,
                                                1,
                                                &BleCoCContextPeriph.Channel_ID);
            if(ret != BLE_STATUS_SUCCESS)
            {
          ...
          {
            APP_DBG_MSG("Invalid event type\n");
            return;
          }
          
          BleCoCContextPeriph.Conn_Handle = p_l2cap_cos_connection->Connection_Handle;
          BleCoCContextPeriph.Peer_Max_Transmission_Unit = p_l2cap_cos_connection->Peer_MTU;
          BleCoCContextPeriph.SPSM = p_l2cap_cos_connection->SPSM;  
          
          HandleNotification.CoC_Evt_Opcode = EXCHANGE_ATT_MTU;
          HandleNotification.ConnectionHandle = BleCoCContextPeriph.Conn_Handle;
          COC_PERIPH_APP_Notification(&HandleNotification);
        }
        break;
      ...
      case ACI_L2CAP_COS_SDU_DATA_RX_VSEVT_CODE:
        {
          aci_l2cap_cos_sdu_data_tx_event_rp0 *p_l2cap_sdu_data_tx_resp;
          p_l2cap_sdu_data_tx_resp = (aci_l2cap_cos_sdu_data_tx_event_rp0 *) p_blecore_evt->data;
          ...
          HandleNotification.DataLength = p_l2cap_sdu_data_tx_resp->SDU_Length;
          
          ret = aci_l2cap_cos_sdu_data_extract(p_l2cap_sdu_data_tx_resp->Connection_Handle, 
                                           p_l2cap_sdu_data_tx_resp->CID, 
                                           MAX_SDU_SIZE,
...
static void Adv_Request(APP_BLE_ConnStatus_t NewStatus)
{
  ...
  {
    APP_DBG_MSG("==>> aci_gap_set_advertising_configuration - Success\n");
  }
  
  /* Set Advertising data */  
  ret = aci_gap_set_advertising_data(0, ADV_COMPLETE_DATA, sizeof(a_AdvData), (uint8_t*) a_AdvData);
  
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("==>> aci_gap_set_advertising_data - fail, result: 0x%02X\n", ret);
  }
  else
  {
    APP_DBG_MSG("==>> aci_gap_set_advertising_data - Success\n");
  }
...
void BLE_SVC_L2CAP_Conn_Update(uint16_t ConnectionHandle, uint16_t conn1, uint16_t conn2)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("BLE_SVC_L2CAP_Conn_Update(), Failed: 0x%02X\n", ret);
  }
  else
  {
...
void APPE_Button2Action(void)
{
  ...
  {
    APP_DBG_MSG("**PERIPH SECURITY REQ \n");
    UTIL_SEQ_SetTask(1 << CFG_TASK_PERIPH_SECURITY_REQ_ID, CFG_SEQ_PRIO_0);
  }
