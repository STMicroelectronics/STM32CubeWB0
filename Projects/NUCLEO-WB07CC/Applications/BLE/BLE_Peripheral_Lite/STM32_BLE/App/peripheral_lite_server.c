...
typedef struct{
  uint16_t  Peripheral_lite_serverSvcHdle;				/**< Peripheral_lite_server Service Handle */
...
}PERIPHERAL_LITE_SERVER_Context_t;
...
static PERIPHERAL_LITE_SERVER_Context_t PERIPHERAL_LITE_SERVER_Context;
...
/*
 * UUIDs for Peripheral Lite Server service
 */
#define PERIPHERAL_LITE_SERVER_UUID			0x8f,0xe5,0xb3,0xd5,0x2e,0x7f,0x4a,0x98,0x2a,0x48,0x7a,0xcc,0x40,0xfe,0x00,0x00
...
/* Peripheral Lite Server service LED_C (write without response), SWITCH_C (notification) characteristics definition */
static const ble_gatt_chr_def_t peripheral_lite_server_chars[] = {
	{
...
/* Peripheral Lite Server service definition */
static const ble_gatt_srv_def_t peripheral_lite_server_service = {
   .type = BLE_GATT_SRV_PRIMARY_SRV_TYPE,
   .uuid = BLE_UUID_INIT_128(PERIPHERAL_LITE_SERVER_UUID),
   .chrs = {
       .chrs_p = (ble_gatt_chr_def_t *)peripheral_lite_server_chars,
       .chr_count = 2U,
   },
};
 ...
 */
static BLEEVT_EvtAckStatus_t PERIPHERAL_LITE_SERVER_EventHandler(aci_blecore_event *p_evt)
{
  ...
  PERIPHERAL_LITE_SERVER_NotificationEvt_t notification;
  ...
  switch(p_evt->ecode)
  {
    case ACI_GATT_SRV_ATTRIBUTE_MODIFIED_VSEVT_CODE:
    {
      ...
      if(p_attribute_modified->Attr_Handle == (PERIPHERAL_LITE_SERVER_Context.Switch_CCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
      {
        ...
        case (!BLE_GATT_SRV_CCCD_NOTIFICATION):
          ...
          notification.EvtOpcode = PERIPHERAL_LITE_SERVER_SWITCH_C_NOTIFY_DISABLED_EVT;
          PERIPHERAL_LITE_SERVER_Notification(&notification);
        ...
        case BLE_GATT_SRV_CCCD_NOTIFICATION:
          ...
          notification.EvtOpcode = PERIPHERAL_LITE_SERVER_SWITCH_C_NOTIFY_ENABLED_EVT;
          PERIPHERAL_LITE_SERVER_Notification(&notification);
      ...
      else if(p_attribute_modified->Attr_Handle == (PERIPHERAL_LITE_SERVER_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;

        notification.EvtOpcode = PERIPHERAL_LITE_SERVER_LED_C_WRITE_NO_RESP_EVT;
        ...
        PERIPHERAL_LITE_SERVER_Notification(&notification);
      } /* if(p_attribute_modified->Attr_Handle == (PERIPHERAL_LITE_SERVER_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
    ...
    case ACI_GATT_SRV_READ_VSEVT_CODE :
    {
	  ...
	  if(p_read->Attribute_Handle == (PERIPHERAL_LITE_SERVER_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
	  {
		return_value = BLEEVT_Ack;
    ...
    case ACI_GATT_SRV_WRITE_VSEVT_CODE:
    {
      ...
      if(p_write->Attribute_Handle == (PERIPHERAL_LITE_SERVER_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
      {
        return_value = BLEEVT_Ack;
  ...
  return(return_value);
}/* end PERIPHERAL_LITE_SERVER_EventHandler */
...
void PERIPHERAL_LITE_SERVER_Init(void)
{
  ...
  UNUSED(PERIPHERAL_LITE_SERVER_Context);
   ...
   */
  BLEEVT_RegisterGattEvtHandler(PERIPHERAL_LITE_SERVER_EventHandler);

  ret = aci_gatt_srv_add_service((ble_gatt_srv_def_t *)&peripheral_lite_server_service);

  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_srv_add_service command: Peripheral_Lite_Server, error code: 0x%x \n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_srv_add_service command: Peripheral_Lite_Server \n");
  }

  PERIPHERAL_LITE_SERVER_Context.Peripheral_lite_serverSvcHdle = aci_gatt_srv_get_service_handle((ble_gatt_srv_def_t *) &peripheral_lite_server_service);
  PERIPHERAL_LITE_SERVER_Context.Led_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&peripheral_lite_server_chars[0]);
  PERIPHERAL_LITE_SERVER_Context.Switch_CCharHdle = aci_gatt_srv_get_char_decl_handle((ble_gatt_chr_def_t *)&peripheral_lite_server_chars[1]);
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail registering Peripheral_Lite_Server handlers\n");
  }
...
tBleStatus PERIPHERAL_LITE_SERVER_UpdateValue(PERIPHERAL_LITE_SERVER_CharOpcode_t CharOpcode, PERIPHERAL_LITE_SERVER_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case PERIPHERAL_LITE_SERVER_LED_C:
...
tBleStatus PERIPHERAL_LITE_SERVER_NotifyValue(PERIPHERAL_LITE_SERVER_CharOpcode_t CharOpcode, PERIPHERAL_LITE_SERVER_Data_t *pData, uint16_t ConnectionHandle)
{
  ...
  switch(CharOpcode)
  {
    ...
    case PERIPHERAL_LITE_SERVER_SWITCH_C:
      ret = aci_gatt_srv_notify(ConnectionHandle,
                                BLE_GATT_UNENHANCED_ATT_L2CAP_CID,
                                PERIPHERAL_LITE_SERVER_Context.Switch_CCharHdle + 1,
                                GATT_NOTIFICATION,
